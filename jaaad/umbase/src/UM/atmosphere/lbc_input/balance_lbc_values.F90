...
!                       -reset w
!
! Original Programmer : Code based on SET_LATERAL_BOUNDARIES at 6.4. 
! Current code owner  : Peter Lean
!
! Arguments:
...
#include "c_epslon.h"
      ...
      REAL, INTENT(IN) ::                                               &  
       ...
       , two_omega      &  ! 2 * angular velocity of planet
...
!========================================================================
      ...
      DO GJ=1-HALO_J,global_rows+HALO_J
        DO GI=1-HALO_I,global_row_length+HALO_I
          ...
          longitude_rot = (base_lambda + (gi-1) * delta_lambda)
          latitude_rot  = (base_phi + (gj-1) * delta_phi)

          f1_at_theta(gi,gj) = - two_omega *                            &
                           SIN(longitude_rot) *COS(lat_rot_NP)

          f2_at_theta(gi,gj) = two_omega *                              &
                          ( COS(latitude_rot) * SIN(lat_rot_NP) -       &
                            SIN(latitude_rot) *                         &
                           COS(longitude_rot) * COS(lat_rot_NP) )
