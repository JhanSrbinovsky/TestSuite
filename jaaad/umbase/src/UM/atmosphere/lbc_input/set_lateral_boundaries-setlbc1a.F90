...
! independently.
! Original Programmer : Based on code written by Mark H. Mawson
! Current code owner  : Paul Burton
!
! History
! Date       Version    Comment
! ----       -------    -------
! 02/03/00   5.1        New deck created.                P.Burton
! 23/10/00   5.2        - Fix bug in weights for Eastern corners
!                       - Introduce IF tests inside loops applying LBCs
!                         to avoid doing manipulations of FIELD or LBC
!                         variables containing unset (possibly NaN)
!                         values.                         P.Burton
! 06/02/03   5.5        correct comments                 A.Malcolm
! 09/10/03   6.0        correct error found when running 1
!                       processor East-west                 A.Malcolm
! 02/03/05   6.2        change order of calcs to stop possible
!                       memory overwriting.              A.Malcolm
!
! Arguments:
...
! Finally, we need to check if this is U-grid data and if so, if
! this processor is at the far right (ie. East) of the grid. In
...
!   on each row has not been updated, so we fill it with zeros here
     ...
     &     (FLD_TYPE  ==  fld_type_u)) THEN
        DO K=1,LEVELS
          ...
          ENDDO ! J
        ENDDO ! K
      ENDIF ! If at East and a U field
