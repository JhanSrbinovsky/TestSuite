...
!                       Surface Fields
     &   , LAND_ALBEDO, L_MOSES_II, L_CTILE, L_USE_SPEC_SEA             &
...
!     Modules included.
      ...
      USE solinc_data, ONLY:                                            &
        sol_bearing, lg_orog_corr, orog_corr,                           &
...
!             Cloud Overlap used by R2_CLOUD_LEVEL_DIAG
     ...
     &        , NPD_CLOUD_COMPONENT, SW_SPECTRUM%NPD_BAND)              &
...
!             DIRECT ALBEDO FIELD
     &   , ALBEDO_FIELD_DIFF(NPD_PROFILE, SW_SPECTRUM%NPD_BAND)         &
!             DIFFUSE ALBEDO FIELD
     &   , ALBEDO_FIELD_DIR(NPD_PROFILE, SW_SPECTRUM%NPD_BAND)          &
!             DIRECT ALBEDO FIELD
     &   , EMISSIVITY_FIELD(NPD_PROFILE, SW_SPECTRUM%NPD_BAND)          &
!             EMISSIVITY FIELD
     &   , ALBEDO_SEA_DIFF_G(NPD_PROFILE, SW_SPECTRUM%NPD_BAND)         &
!             GATHERED DIFFUSE ALBEDO FOR OPEN SEA
     &   , ALBEDO_SEA_DIR_G(NPD_PROFILE, SW_SPECTRUM%NPD_BAND)          &
...
!             UPWARD UV-FLUX
     &   , UV_FLUX_NET(NPD_PROFILE, 0: NPD_LAYER)
!             NET UV-FLUX
...
!             WEIGHTS FOR EACH BAND FOR REGION IN UV INTERVAL
!
...
! DEPENDS ON: r2_compare_proc
      CALL R2_COMPARE_PROC(IERR, SW_SPECTRUM%L_PRESENT                  &
     &   , SW_CONTROL%L_RAYLEIGH, SW_CONTROL%L_GAS                      &
     &   , SW_CONTROL%L_CONTINUUM, SW_CONTROL%L_DROP                    &
     &   , SW_CONTROL%L_AEROSOL, SW_CONTROL%L_AEROSOL_CCN               &
     &   , SW_CONTROL%L_ICE                                             &
...
! DEPENDS ON: r2_set_surface_field_sw
     ...
     &     SW_SPECTRUM%N_BAND                                           &
     &   , NLIT, LIST                                                   &
     &   , I_SURFACE, SW_SPECTRUM%I_SPEC_SURFACE                        &
     &   , SW_SPECTRUM%L_SURFACE                                        &
     ...
     &   , SW_SPECTRUM%NPD_BAND, SW_SPECTRUM%NPD_SURFACE                &
     &   )
!
...
! DEPENDS ON: r2_set_aerosol_field
     ...
     &      , NLIT, NLEVS, N_LAYER                                      &
     &      , SW_SPECTRUM%N_AEROSOL, SW_SPECTRUM%TYPE_AEROSOL           &
     &      , LIST, SW_CONTROL%L_EXTRA_TOP                              &
     &      , L_CLIMAT_AEROSOL, L_CLIM_AERO_HGT, L_HadGEM1_Clim_Aero    &
...
! DEPENDS ON: r2_set_cloud_parametrization
     ...
     &   , SW_SPECTRUM%DROP_PARAMETER_LIST                              &
     &   , SW_SPECTRUM%DROP_PARM_MIN_DIM                                &
     &   , SW_SPECTRUM%DROP_PARM_MAX_DIM                                &
     &   , SW_SPECTRUM%L_ICE_TYPE                                       &
     &   , SW_SPECTRUM%I_ICE_PARAMETRIZATION                            &
     &   , SW_SPECTRUM%ICE_PARAMETER_LIST                               &
     &   , SW_SPECTRUM%ICE_PARM_MIN_DIM                                 &
     &   , SW_SPECTRUM%ICE_PARM_MAX_DIM                                 &
     &   , I_CONDENSED_PARAM, CONDENSED_PARAM_LIST                      &
     &   , CONDENSED_MIN_DIM, CONDENSED_MAX_DIM                         &
     &   , SW_SPECTRUM%NPD_BAND                                         &
     &   , SW_SPECTRUM%NPD_DROP_TYPE                                    &
     &   , SW_SPECTRUM%NPD_ICE_TYPE                                     &
...
!     CHECK THAT A VALID NUMBER HAS BEEN SUPPLIED FOR THE SOLVER.
      IF ( (SW_CONTROL%I_SOLVER /= IP_SOLVER_PENTADIAGONAL).AND.        &
     &     (SW_CONTROL%I_SOLVER /= IP_SOLVER_MIX_11).AND.               &
     &     (SW_CONTROL%I_SOLVER /= IP_SOLVER_MIX_DIRECT).AND.           &
     &     (SW_CONTROL%I_SOLVER /= IP_SOLVER_MIX_DIRECT_HOGAN).AND.     &
     &     (SW_CONTROL%I_SOLVER /= IP_SOLVER_HOMOGEN_DIRECT).AND.       &
     &     (SW_CONTROL%I_SOLVER /= IP_SOLVER_TRIPLE).AND.               &
...
!       SELECT A CLEAR-SKY SOLVER TO MATCH THE MAIN SOLVER.
        IF (SW_CONTROL%I_SOLVER == IP_SOLVER_PENTADIAGONAL) THEN
           I_SOLVER_CLEAR=IP_SOLVER_PENTADIAGONAL
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_MIX_11) THEN
           I_SOLVER_CLEAR=IP_SOLVER_PENTADIAGONAL
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_MIX_DIRECT) THEN
           I_SOLVER_CLEAR=IP_SOLVER_HOMOGEN_DIRECT
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_MIX_DIRECT_HOGAN) THEN
           I_SOLVER_CLEAR=IP_SOLVER_HOMOGEN_DIRECT
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_HOMOGEN_DIRECT) THEN
           I_SOLVER_CLEAR=IP_SOLVER_HOMOGEN_DIRECT
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_TRIPLE) THEN
           I_SOLVER_CLEAR=IP_SOLVER_HOMOGEN_DIRECT
        ELSE IF (SW_CONTROL%I_SOLVER == IP_SOLVER_TRIPLE_HOGAN) THEN
           I_SOLVER_CLEAR=IP_SOLVER_HOMOGEN_DIRECT
        ENDIF
!
      ENDIF
...
!     SET PROPERTIES FOR INDIVIDUAL BANDS.
      DO I=1, SW_SPECTRUM%N_BAND
         WEIGHT_BAND(I)=1.0E+00
         I_GAS_OVERLAP(I)=SW_CONTROL%I_GAS_OVERLAP
      ENDDO
!
!     Set the treatment of scattering.
      DO I=1, SW_SPECTRUM%N_BAND
         I_SCATTER_METHOD_BAND(I)=SW_CONTROL%I_SCATTER_METHOD
      ENDDO
!
!
!
...
!                       Logical Flags for Processes
     ...
     &   , SW_CONTROL%L_CLOUD, L_DROP, L_ICE, L_PC2                     &
!                       Angular Integration
     &   , SW_CONTROL%I_ANGULAR_INTEGRATION                             &
     &   , SW_CONTROL%I_2STREAM                                         &
     &   , SW_CONTROL%L_2_STREAM_CORRECT                                &
     &   , SW_CONTROL%L_RESCALE, N_ORDER_GAUSS                          &
!                       Treatment of Scattering
...
!                       General Spectral Properties
     &   , SW_SPECTRUM%N_BAND, 1, SW_SPECTRUM%N_BAND, WEIGHT_BAND       &
!                       General Atmospheric Properties
     &   , NLIT, N_LAYER                                                &
     &   , SW_CONTROL%L_LAYER, SW_CONTROL%L_CLOUD_LAYER                 &
     &   , P, T, DUMMY1D, DUMMY1D, DUMMY1D, DUMMY2D, D_MASS             &
!                       Spectral Region
     &   , SW_CONTROL%ISOLIR                                            &
!                       Solar Fields
     &   , SEC_0, SOLAR_INCIDENT_NORM                                   &
     &   , SW_SPECTRUM%SOLAR_FLUX_BAND                                  &
     &   , SW_SPECTRUM%RAYLEIGH_COEFFICIENT                             &
!                       Infra-red Fields
     &   , SW_SPECTRUM%N_DEG_FIT                                        &
     &   , SW_SPECTRUM%THERMAL_COEFFICIENT                              &
     &   , SW_SPECTRUM%T_REF_PLANCK, .FALSE.                            &
!                       Gaseous Absorption
     &   , SW_SPECTRUM%N_ABSORB, I_GAS_OVERLAP, I_GAS, GAS_MIX_RATIO    &
     ...
     &   , L_WENYI, SW_SPECTRUM%SCALE_VECTOR                            &
...
!                       Surface Fields
     &   , SW_SPECTRUM%L_SURFACE, I_SURFACE                             &
     &   , SW_SPECTRUM%I_SPEC_SURFACE                                   &
     &   , SW_SPECTRUM%SURFACE_ALBEDO                                   &
     &   , ALBEDO_FIELD_DIFF, ALBEDO_FIELD_DIR                          &
     &   , SW_SPECTRUM%N_DIR_ALBEDO_FIT                                 &
     &   , SW_SPECTRUM%DIRECT_ALBEDO_PARM                               &
     &   , SW_SPECTRUM%EMISSIVITY_GROUND                                &
     &   , EMISSIVITY_FIELD                                             &
!                       Continuum Absorption
     ...
     &   , SW_SPECTRUM%P_REF_CONTINUUM                                  &
     &   , SW_SPECTRUM%T_REF_CONTINUUM                                  &
!                       Properties of Aerosols
     &   , SW_SPECTRUM%N_AEROSOL                                        &
     &   , AEROSOL_MIX_RATIO                                            &
     &   , SW_SPECTRUM%AEROSOL_ABSORPTION                               &
     &   , SW_SPECTRUM%AEROSOL_SCATTERING                               &
     &   , SW_SPECTRUM%AEROSOL_PHASE_FNC(:,1,:,:)                       &
     &   , SW_SPECTRUM%I_AEROSOL_PARAMETRIZATION                        &
     &   , SW_SPECTRUM%NHUMIDITY, SW_SPECTRUM%HUMIDITIES                &
     &   , SW_SPECTRUM%TYPE_AEROSOL, L_USE_CLEARRH                      &
!                       Aerosol optical depth
!                       (computed in the LW: dummy values for SW)
     &   , SW_SPECTRUM%N_AOD_WAVEL, .FALSE., DUMMY2D, .FALSE., DUMMY2D  &
...
!                       Dimensions of Arrays
     ...
     &   , SW_SPECTRUM%NPD_ESFT_TERM, SW_SPECTRUM%NPD_SCALE_FNC         &
     &   , SW_SPECTRUM%NPD_SCALE_VARIABLE, SW_SPECTRUM%NPD_CONTINUUM    &
     &   , SW_SPECTRUM%NPD_AEROSOL_SPECIES                              &
     &   , SW_SPECTRUM%NPD_HUMIDITIES                                   &
     &   , SW_SPECTRUM%NPD_CLOUD_PARAMETER                              &
     &   , SW_SPECTRUM%NPD_THERMAL_COEFF, SW_SPECTRUM%NPD_SURFACE       &
     &   , SW_SPECTRUM%NPD_ALBEDO_PARM, SW_SPECTRUM%NPD_AOD_WAVEL       &
     &   )
      IF (IERR /= I_NORMAL) RETURN
!
!
