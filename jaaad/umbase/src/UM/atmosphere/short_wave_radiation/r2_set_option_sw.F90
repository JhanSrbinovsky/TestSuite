...
! *****************************COPYRIGHT*******************************
!
!+ Subroutine for algorithmic initializations
!
! Purpose:
...
!
!- ---------------------------------------------------------------------
      subroutine r2_set_option_sw(ierr                                  &
     &  , n_layer, n_band                                               &
     &  , isolir                                                        &
     ...
     &  , l_solar_out_clear, l_surf_down_clr, l_surf_up_clr             &
     &  , l_clear_hr, l_clear                                           &
     ...
     &  , i_scatter_method, i_scatter_method_list                       &
     &  , l_rad_tile                                                    &
     &  , weight_band                                                   &
...
!           Flag to include the direct effect of biogenic aerosols
     &  , l_murk_rad
!           Flag to include the mesoscale aerosol
...
!           Flag for clear-sky downward surface flux
     &  , l_surf_up_clr                                                 &
!           Flag for clear-sky upward surface flux
     &  , l_solar_out_clear                                             &
!           Flag for clear-sky outgoing flux
     &  , l_clear_hr
...
!       clouds switched off.
        l_clear=.false.
        if (l_solar_out_clear   .OR.                                    &
     &      l_surf_down_clr     .OR.                                    &
     &      l_surf_up_clr       .OR.                                    &
     &      l_clear_hr ) then
          ierr=i_err_fatal
! DEPENDS ON: ereport
          call Ereport('set_option_sw', ierr                            &
...
!       to each band.
        i_scatter_method_list(i)=i_scatter_method
      enddo
!
!
...
! Code changed by JCT on the 19/04/06
!        nd_layer_clr=n_cloud_top_global-1
         nd_layer_clr=n_cloud_top_global
!
!
!
      return
      END SUBROUTINE r2_set_option_sw
#endif
