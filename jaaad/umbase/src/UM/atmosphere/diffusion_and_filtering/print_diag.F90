...
! ----    --------   -------
!  5.5    28/02/03   Original code was WDIVDIA            Terry Davies
!  6.2    28/09/05   Re-written code and extra features   Terry Davies
!
! Code Description:
...
!          global min after calling gc_rin
            kminr = kminr + 1
...
! ----------------------------------------------------------------------
        if (L_print_w .or. L_print_wmax ) then
          ...
          Do   k = 1, model_levels - 1
            ...
            max_w_pe(k) = w(max_indices(1),max_indices(2),k)
...
!  Initialise pointers in arrays for summing over pe's
        ...
        if ( L_diag_wind .or. L_print_max_wind ) then
          ...
          allocate ( u1 (1-off_x:row_length+off_x, 1-off_y:rows+off_y,  &
     &                   model_levels) )
          allocate ( v1 (1-off_x:row_length+off_x, 1-off_y:n_rows+off_y,&
     &                   model_levels) )
          u1(:,:,:)=u(:,:,:)
          v1(:,:,:)=v(:,:,:)
...
! DEPENDS ON: v_to_p
          CALL v_to_p(v1, row_length, rows, n_rows, model_levels,       &
     &              off_x, off_y, model_domain, at_extremity, v_rho)

! DEPENDS ON: u_to_p
          CALL u_to_p(u1, row_length, rows, model_levels,               &
...
!-------------------------------------------------------------------
     ...
     &                             v_rho(i,j,k) * v_rho(i,j,k) )
              ...
              j_max_wind(k) = max_indices(2)
              max_wind_pe(k) = work1(max_indices(1),max_indices(2))
...
! ----------------------------------------------------------------------
      ...
      if( kr > 0 ) call gc_rsumr(kr, n_proc, info, sumr)
...
! ----------------------------------------------------------------------
 ...
 995   FORMAT(1X, E11.3, I4, I5, F7.1, A7, F7.1, A7, E11.3, I5, I6)
 996   FORMAT(1X, F11.2, I8, F8.1, A7, F8.1, A7, F11.2, I6)
 997   FORMAT(1X, I4, I6, E11.3, I5, F6.1, A7, F6.1, A7, E11.3, I6)
 998   FORMAT(1X, I4, I6, E11.3, I5, F6.1, A7, F6.1, A7, E11.3, I6)
 999   FORMAT(1X, I4, E11.3, I5, F6.1, A7, F6.1, A7, E11.3, I6)

      return      ! End of routine Print_diag
