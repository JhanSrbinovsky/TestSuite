...
!                             ! IN boundary layer height from previous
!                             !    timestep (not used in 6A scheme)
...
!                                     k-1/2 above the surface (m).
     &,DELTAP(row_length,rows,BL_LEVELS)                                &
                              ! IN Pressure difference between
!                             !    half-levels (Pa).
...
!                                   turbulent boundary layer.
     &,Z_LCL(row_length,rows)                                           &
                              ! INOUT Height of lifting condensation
!                                   level.
...
!                             ! OUT a fraction of the timestep
     &,ZHPAR(row_length,rows) ! INOUT Height of top of NTPAR
...
!*CALL C_GAMMA
     ...
     &)
! GMM DECE (KMKHZ)      REAL A_PLUME,B_PLUME,A_GRAD_ADJ,MAX_T_GRAD
...
!                                     ! in-cloud liquid cloud water in a
...
!                                      !  interface.
     &,DB_FOR_FLUX(row_length,rows,2:BL_LEVELS)                         &
                                  ! Buoyancy jump in flux
!                                 ! integral calculation (m/s2)
     &,DF_OVER_CP(row_length,rows,BL_LEVELS)                            &
                                  ! Radiative flux change over layer
!                                 ! divided by c_P.
     &,DF_TOP_OVER_CP(row_length,rows)                                  &
                                      ! Radiative flux change at cloud t
!                                     ! divided by c_P.
...
!                                     ! dilution from level 1.
     &,SVL_KM1(row_length,rows)                                         &
                                      ! Liquid/frozen water virtual
!                                     ! static energy over CP for the
!                                     ! last layer considered.
     &,ENV_SVL_KM1(row_length,rows)                                     &
                                      ! Density potential temperature
!                                     ! for last layer considered
     &,PAR_SVL_KM1(row_length,rows)                                     &
                                      ! Density potential temperature
                                ! of parcel at last level considered
!                                     !
...
!                                     ! surface mixed-layer.
     &,SVL(row_length,rows,BL_LEVELS)                                   &
                                      ! Liquid/frozen water virtual
!                                     ! temperature over CP.
     &,DSVL_KP2(row_length,rows)                                        &
                                      ! Liquid/frozen water virtual
!                                     ! static energy over CP gradient
...
!                                     !    vertical advection (s^-1)
     ...
     &,TV_PERT(row_length,rows)                                         &
                                      ! threshold Tv of parcel
     ...
     &,NTML_SAVE(row_length,rows)                                       &
                                      ! Copy of NTML
     &,K_PLUME(row_length,rows)       ! Start grid-level for
...
!                        ! interface.
     &,VAP_PRESS                                                        &
                         ! Vapour pressure.
     &,GRAD_CLD                                                         &
                         ! Humidity gradient in layer above LCL.
     &,GRAD_SUB                                                         &
                         ! Humidity gradient in layer below LCL.
     &,Q_VAP_PARC                                                       &
...
!                        ! temperature of parcel
     &,DSVL_KP1                                                         &
                         ! s_VL gradient between previous layers
     &,RHO_DZ                                                           &
                         ! rho*dz
     &,SVL_LAPSE                                                        &
     ...
     &,W_S_CUBED   ! convective velocity scale
! end
!
      INTEGER                                                           &
     & I                                                                &
...
!-----------------------------------------------------------------------
! Calculate the Standard Deviation of level 1 virtual temperature (K).
!-----------------------------------------------------------------------
      do j=1,rows
      do i=1,row_length
...
!-----------------------------------------------------------------------
      ...
      ENDDO
!-----------------------------------------------------------------------
! Reset ZH, Z_LCL and ZHPAR to their appropriate half level values
! (May not be required if BLT_DIAG is called just before BL scheme)
!-----------------------------------------------------------------------

      do j=1,rows
      do i=1,row_length
        ZH(I,j) = Z_HALF(I,j,NTML(I,j)+1)
        IF (NTPAR(I,j)  <=  MBL) THEN
          ZHPAR(I,j) = Z_HALF(I,j,NTPAR(I,j)+1)
        ENDIF
...
!-----------------------------------------------------------------------
        ...
        IF ( ( (L_conv4a .AND. L_SHALLOW(I,j))                          &
!            ! shallow cumulus layer within BL_LEVELS (A05_4A)
     &       .OR. (.NOT. L_conv4a .AND. CUMULUS(I,j)) )                 &
!            ! cumulus layer
     &       .AND. NTPAR(I,j)  <   BL_LEVELS ) THEN
!            ! inversion found
...
!         ! Combine separate LW and SW increments into a total increment
          DF_OVER_CP(I,j,K) = - ( RAD_HR(I,j,K,1) + RAD_HR(I,j,K,2) )   &
     &                        * RHO_DRY_TQ(I,j,K) * DZL(I,j,K)
!         !-------------------------------------------------------------
!         ! Find the level with the greatest radiative flux jump and
...
!         !-------------------------------------------------------------
     ...
     &         .AND. DF_OVER_CP(I,j,K)  >   DF_DSCT_OVER_CP(I,j) ) THEN
            K_CLOUD_DSCT(I,j) = K
            DF_DSCT_OVER_CP(I,j) = DF_OVER_CP(I,j,K)
          ENDIF
...
!     perturbation to the cloud-top SVL (level NTDSC), based roughly
...
! Thus use cloud-top radiative forcing as follows:
     ...
     &     - CT_RESID * DZL(I,j,K)*DF_DSCT_OVER_CP(I,j) / ( 50.*RHO_DZ )
      ...
      do i=1,row_length
       IF ( K <  NTDSC(I,j)                                             &
                                    ! layer must at least 2 levels deep
     &             .AND. SVL_PLUME(I,j)-SVL(I,j,K)  <   0.01 ) THEN
        NBDSC(I,j) = K
       ENDIF
      ENDDO
      ENDDO
...
!..length-scale used in the entrainment inputs.
...
!..initially and use grid-level jumps
        ...
        IF ( .NOT.CUMULUS(I,j) .AND. K  >   1                           &
     &                       .AND. K  <=  BL_LEVELS-2 ) THEN
     ...
     &                  ( SVL(I,j,K+3) - SVL(I,j,K+2) ) * RDZ(I,j,K+3) )
              ELSE
                SVL_LAPSE = 0.0
              ENDIF
              IF ( K  >=  K_PLUME(I,j)+1 ) THEN
                SVL_LAPSE_BASE = ( SVL(I,j,K)-SVL(I,j,K-1) )*RDZ(I,j,K)
              ELSE
                SVL_LAPSE_BASE = 0.0
              ENDIF
...
!               ! lower NTML but set ZH only fractionally (DZ_DISC_MIN)
...
!..DZ_DISC small, ZH close to level above: use double grid-level jumps
          ...
          ENDIF  ! SVL increasing
         ENDIF ! not cumulus and not at top of bl_levels
!-----------------------------------------------------------------------
...
!               ! lower NTDSC but set ZHSC only fractionally
!               ! (DZ_DISC_MIN) below the top of the inversion level.
...
!               ! Note that all but DZ_DISC_MIN of this layer will be
!               ! added back on to DSCDEPTH a few lines below
                 ENDIF
                ENDIF
...
!..DZ_DISC small, ZHSC close to level above: use double jumps
          ...
          ENDIF  ! SVL increasing
          ENDIF ! test on K LT BL_LEVELS-2
         ENDIF ! test on DSC
      ENDDO
      ENDDO
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
          DTL = TL(I,j,K) - TL(I,j,KM1) + GRCP/RDZ(I,j,K)
                                ! Used in P243.C2
          IF (K  <=  NTML(I,j)) THEN
            DTL = DTL - GRAD_T_ADJ(I,j)/RDZ(I,j,K)
            DQW = DQW - GRAD_Q_ADJ(I,j)/RDZ(I,j,K)
          ENDIF
!
...
!             ! ensure DB>0 so that entrainment is non-zero
     ...
     &                  - A_DQSDTM(I,j,K-1)*DTL_SML(I,j) )
          ...
          ENDIF  ! disc inversion diagnosed
...
! use mixed-layer average of buoyancy parameters
     ...
     &       (BQM_CLD(I,j,1)+BQM_CLD(I,j,NTDSC(I,j)))*FQW(I,j,1) )
      ...
      ENDDO
!-----------------------------------------------------------------------
!L 3.0aa Calculate Sc layer cloud depth (not cloud fraction weighted):
!       this section replaces section 3.2 as ZC's are now used in
!       calculation of entrainment param inputs.
!       If DSC(I,j)=.FALSE. then NTDSC=0 and ZC_DSC remains equal to 0.
!       Consistent with the decoupled cloud identification algorithm,
!       ZCs are only calculated if CF > SC_CFTOL.
!-----------------------------------------------------------------------
!
!     Initialise variables
!
...
!
      DO K=BL_LEVELS,2,-1
        ...
        do j=1,rows
        do i=1,row_length
          IF ( (K  ==  NTDSC(I,j)) .AND. (CF(I,j,K)  >   SC_CFTOL) )    &
     &      CLOUD_BASE(I,j) = .FALSE.
          IF ( (K  <=  NTDSC(I,j)) .AND. (CF(I,j,K)  >   SC_CFTOL) .AND.&
     &        .NOT.CLOUD_BASE(I,j) ) THEN
            ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,K)
            IF (CF(I,j,KM1)  >   SC_CFTOL) THEN
              ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,K)
            ELSE
              ZC_DSC(I,j) = ZC_DSC(I,j) +                               &
     &             MIN( 0.5*(DZL(I,j,K) + DZL(I,j,KM1)) * CFL(I,j,K) ,  &
     &                          QCL(I,j,K) / DQCLDZ(I,j,K) ) / CF(I,j,K)
              IF ( DQCFDZ(I,j,K)  >   0.0 ) THEN
                ZC_DSC(I,j) = ZC_DSC(I,j) +                             &
     &             MIN( 0.5*(DZL(I,j,K) + DZL(I,j,KM1)) * CFF(I,j,K) ,  &
     &                          QCF(I,j,K) / DQCFDZ(I,j,K) ) / CF(I,j,K)
              ELSE
                ZC_DSC(I,j) = ZC_DSC(I,j) +                             &
     &          0.5*(DZL(I,j,K) + DZL(I,j,KM1)) * CFF(I,j,K) / CF(I,j,K)
              ENDIF
              CLOUD_BASE(I,j) = .TRUE.
            ENDIF
          ENDIF
      ...
      do j=1,rows
      do i=1,row_length
        IF ( DSC(I,j) .AND. .NOT.CLOUD_BASE(I,j)                        &
     &              .AND. (CF(I,j,1)  >   SC_CFTOL) ) THEN
          ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,1) +                  &
     &              MIN( 0.5*DZL(I,j,1)*CFL(I,j,1) ,                    &
     &                    QCL(I,j,1) / DQCLDZ(I,j,1) ) / CF(I,j,1)
          IF ( DQCFDZ(I,j,K)  >   0.0 ) THEN
            ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,1) +                &
     &              MIN( 0.5*DZL(I,j,1)*CFF(I,j,1) ,                    &
     &                    QCF(I,j,1) / DQCFDZ(I,j,1) ) / CF(I,j,1)
          ELSE
            ZC_DSC(I,j) = ZC_DSC(I,j) +                                 &
     &                     0.5*DZL(I,j,1) * CFF(I,j,1) / CF(I,j,1)
          ENDIF
          CLOUD_BASE(I,j) = .TRUE.
        ENDIF
      ENDDO
...
!     !-----------------------------------------------------------------
      ...
      do i=1,row_length
        IF ( DSC(I,j) ) THEN
         IF ( ZHSC(I,j)  >   Z_HALF(I,j,NTDSC(I,j)+1) )THEN
!..discontinuous inversion found so cloud-top height known
          ZC_DSC(I,j) = ZC_DSC(I,j)                                     &
     &                    + ZHSC(I,j) - Z_HALF(I,j,NTDSC(I,j)+1)
         ELSE
!..no discontinuous inversion so code as before
          IF ( CF(I,j,NTDSC(I,j)+1)  >   SC_CFTOL ) THEN
            K=NTDSC(I,j)+1
            ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,K)
            IF (CF(I,j,K-1)  >   SC_CFTOL) THEN
              ZC_DSC(I,j) = ZC_DSC(I,j) + 0.5*DZL(I,j,K)
            ELSE
              ZC_DSC(I,j) = ZC_DSC(I,j) +                               &
     &           MIN( 0.5*(DZL(I,j,K) + DZL(I,j,K-1)) * CFL(I,j,K) ,    &
     &                        QCL(I,j,K) / DQCLDZ(I,j,K) ) / CF(I,j,K)
              IF ( DQCFDZ(I,j,K)  >   0.0 ) THEN
                ZC_DSC(I,j) = ZC_DSC(I,j) +                             &
     &             MIN( 0.5*(DZL(I,j,K) + DZL(I,j,K-1)) * CFF(I,j,K) ,  &
     &                          QCF(I,j,K) / DQCFDZ(I,j,K) ) / CF(I,j,K)
              ELSE
                ZC_DSC(I,j) = ZC_DSC(I,j) +                             &
     &          0.5*(DZL(I,j,K) + DZL(I,j,K-1)) * CFF(I,j,K) / CF(I,j,K)
              ENDIF
            ENDIF   ! test on cloud fraction
           ENDIF   ! test on cloud fraction
          ENDIF   ! disc inversion
        ENDIF   ! test on DSC
      ENDDO
      ENDDO
!     !-----------------------------------------------------------------
...
!     !-----------------------------------------------------------------
      ...
      ENDDO
!-----------------------------------------------------------------------
!L 3.0b Calculate surface mixed-layer cloud depth (not cloud fraction
!       weighted)
!-----------------------------------------------------------------------
!
      do j=1,rows
...
!
      DO K=BL_LEVELS-1,2,-1
        ...
        do j=1,rows
        do i=1,row_length
          IF ( (K  ==  NTML(I,j)) .AND. (CF(I,j,K)  >   SC_CFTOL) )     &
     &         CLOUD_BASE(I,j) = .FALSE.
          IF ( (K  <=  NTML(I,j)) .AND. (CF(I,j,K)  >   SC_CFTOL) .AND. &
     &        .NOT.CLOUD_BASE(I,j) ) THEN
            ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,K)
            IF (CF(I,j,KM1)  >   SC_CFTOL) THEN
              ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,K)
            ELSE
              ZC(I,j) = ZC(I,j) +                                       &
     ...
     &                        QCF(I,j,K) / DQCFDZ(I,j,K) ) / CF(I,j,K)
              ELSE
                ZC(I,j) = ZC(I,j) + 0.5*(DZL(I,j,K) + DZL(I,j,KM1))     &
     &                                        * CFF(I,j,K) / CF(I,j,K)
              ENDIF
              CLOUD_BASE(I,j) = .TRUE.
            ENDIF
      ...
      do j=1,rows
      do i=1,row_length
        IF ( (CF(I,j,1)  >   SC_CFTOL) .AND. .NOT.CLOUD_BASE(I,j) ) THEN
          ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,1) +                          &
     &               MIN( 0.5*DZL(I,j,1)*CFL(I,j,1) ,                   &
     &                    QCL(I,j,1) / DQCLDZ(I,j,1) ) / CF(I,j,1)
          IF ( DQCFDZ(I,j,K)  >   0.0 ) THEN
            ZC(I,j) = ZC(I,j) +                                         &
     ...
     &                       QCF(I,j,1) / DQCFDZ(I,j,1) ) / CF(I,j,1)
          ELSE
            ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,1) * CFF(I,j,1) / CF(I,j,1)
          ENDIF
          CLOUD_BASE(I,j) = .TRUE.
        ENDIF
      ENDDO
...
!     !-----------------------------------------------------------------
      do j=1,rows
      do i=1,row_length
        IF ( ZH(I,j)  >   Z_HALF(I,j,NTML(I,j)+1) ) THEN
!..discontinuous inversion found so cloud-top height known

          IF ( CF(I,j,NTML(I,j))  >   SC_CFTOL .OR.                     &
     &         CF(I,j,NTML(I,j)+1)  >   SC_CFTOL )                      &
     &             ZC(I,j) = ZC(I,j) + ZH(I,j) - Z_HALF(I,j,NTML(I,j)+1)

        ELSE
!..no discontinuous inversion so code as before
        IF ( CF(I,j,NTML(I,j)+1)  >   SC_CFTOL ) THEN
            K=NTML(I,j)+1
            ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,K)
            IF (CF(I,j,K-1)  >   SC_CFTOL) THEN
              ZC(I,j) = ZC(I,j) + 0.5*DZL(I,j,K)
     ...
     &                        QCF(I,j,K) / DQCFDZ(I,j,K) ) / CF(I,j,K)
              ELSE
                ZC(I,j) = ZC(I,j) + 0.5*(DZL(I,j,K) + DZL(I,j,K-1))     &
     &                                        * CFF(I,j,K) / CF(I,j,K)
              ENDIF
            ENDIF
        ENDIF
...
!
      DO K = 2,BL_LEVELS
        ...
        do i=1,row_length
!-----------------------------------------------------------------------
!- Buoyancy flux factor used in the diagnosis of decoupling:
!-----------------------------------------------------------------------
          ...
          IF (K  <=  NTML(I,j)) THEN
            ...
            DQW = DQW - GRAD_Q_ADJ(I,j)/RDZ(I,j,K)
          ENDIF
          DB_FOR_FLUX(I,j,K) = G * ( BTM(I,j,KM1) * DTL +               &
     &                               BQM(I,j,KM1) * DQW )
!         !----------------------------------------------------------
!         ! add on cloud contribution
!         !----------------------------------------------------------
          IF ( Z_FULL(I,j,K)  >   ZH(I,j)-ZC(I,j) .AND.                 &
     &         Z_FULL(I,j,KM1)  <   ZH(I,j)-ZC(I,j) ) THEN
!           !----------------------------------------------------------
!           !cloud-base within this level so scale cloud contribution
!           !----------------------------------------------------------
            DB_FOR_FLUX(I,j,K) = DB_FOR_FLUX(I,j,K) + G *               &
     &       RDZ(I,j,K) * ( Z_FULL(I,j,K)-(ZH(I,j)-ZC(I,j)) ) *         &
     &             CFM(I,j,KM1)*( (BTM_CLD(I,j,KM1)-BTM(I,j,KM1))*DTL   &
     &                          + (BQM_CLD(I,j,KM1)-BQM(I,j,KM1))*DQW )
          ELSEIF ( Z_FULL(I,j,K)  >   ZHSC(I,j)-ZC_DSC(I,j) .AND.       &
     &             Z_FULL(I,j,KM1)  <   ZHSC(I,j)-ZC_DSC(I,j) ) THEN
!           !----------------------------------------------------------
!           !cloud-base within this level so scale cloud contribution
!           !----------------------------------------------------------
            DB_FOR_FLUX(I,j,K) = DB_FOR_FLUX(I,j,K) + G *               &
     &       RDZ(I,j,K) * ( Z_FULL(I,j,K)-(ZHSC(I,j)-ZC_DSC(I,j)) ) *   &
     &             CFM(I,j,KM1)*( (BTM_CLD(I,j,KM1)-BTM(I,j,KM1))*DTL   &
     &                          + (BQM_CLD(I,j,KM1)-BQM(I,j,KM1))*DQW )
          ELSE
            DB_FOR_FLUX(I,j,K) = DB_FOR_FLUX(I,j,K) + G *               &
...
!         !-------------------------------------------------------------
          IF (DF_OVER_CP(I,j,K)  >   DF_TOP_OVER_CP(I,j)                &
     &                .AND. K  <=  K_RAD_SMLT ) THEN
            K_CLOUD_TOP(I,j) = K
            DF_TOP_OVER_CP(I,j) = DF_OVER_CP(I,j,K)
          ENDIF
...
!..Second the decoupled mixed layer
     ...
     &         DF_NET_DSC(I,j) = DF_NET_DSC(I,j) + DF_OVER_CP(I,j,K)
      ...
      ENDDO
!-----------------------------------------------------------------------
!..Assume RAD_HR(K_cloud_top+2) is representative of clear-air rad
...
!    ! T_FRAC is fraction of timestep inversion spends above Z_HALF(K)
     ...
     &                                           - F_NET_SML(I,j,K) )
          ...
          ENDIF  ! ZH gone above half-level K+1.

        ELSE   ! NOT specifying entrainment flux but KH

!         ! Add to DSC top-driven K-profile (held in RHOKH_TOP)
          RHOKH_TOP(I,j,K) = RHOKH_TOP(I,j,K) + RHOKH_TOP_ENT(I,j)
          RHOKH(I,j,K) = RHOKH_SURF_ENT(I,j)

        ENDIF  ! IF NOT CUMULUS OR COUPLED
!-------------------------------------------------
...
!    ! with ZH above this level
     ...
     &                  TOTHF_INV * ZRZI_DSC(I,j,2) - F_NET_DSC(I,j,K) )
          ...
          ENDIF  ! ZHSC gets above or stays below half-level K+1
...
!        ENDIF
      ...
      ENDDO
!-----------------------------------------------------------------------
!- Save ZC - DSC if it exists, SML otherwise
!-----------------------------------------------------------------------
        do j=1,rows
        do i=1,row_length
          IF ( DSC(I,j) ) THEN
