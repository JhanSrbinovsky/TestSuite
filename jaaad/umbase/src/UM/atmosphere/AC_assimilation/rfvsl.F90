...
!LL version  date
!    4.4 25/9/97 : Allow *L routines into GLOBAL runs. Stuart Bell
!    5.2 15/11/00 : Remove any reference to #def MACRO.  Zoe Gardner
!LL
!LL
!LL   SUBROUTINE RFCSG
!LL   RECURSIVE FILTER - CONSTANT COEFF - FOR A SCALAR GLOBAL FIELD
!LL   VERSION 2.0 STARTED 20/2/92 A LORENC
!LL   DOCUMENTATION:
!LL   THIS IS EXACTLY LIKE RFVSG (SEE BELOW),
...
!LL   SUBROUTINE RFVSL
!LL   RECURSIVE FILTER - VARIABLE COEFF - FOR A SCALAR LTD.AREA FIELD
!LL   VERSION 2.0 STARTED 20/2/92 A LORENC
!LL   DOCUMENTATION:
!       U.M. DOCUMENTATION PAPER 30
!       "THE ANALYSIS CORRECTION DATA ASSIMILATION SCHEME" SECTION 6.5
!
!*L   ARGUMENTS:
      SUBROUTINE RFVSL(F,N_ROWS,ROW_LENGTH,M_GRID,F_BOUNDARY,           &
...
!                                       BC FOR 2 IS ALSO USED FOR >2
         DO      JPT=1,ROW_LENGTH
          F(JPT,1)=F_BOUNDARY+(F(JPT,1)-F_BOUNDARY-                     &
     &     A_NS(JPT,1)**3*(F(JPT,2)-F_BOUNDARY))                        &
...
!L         FILTER N->S PASS
       ...
       DO        JPT=1,ROW_LENGTH
         F(JPT,JROW)=F(JPT,JROW)+                                       &
     &       (F(JPT,JROW-1)-F(JPT,JROW))*A_NS(JPT,JROW)
       ENDDO !)) JPT
       ENDDO !)) JROW
!L
!L***   FILTER S->N             -----------------------------------
...
!                                       BC FOR 2 IS ALSO USED FOR >2
         DO      JPT=1,ROW_LENGTH
          F(JPT,N_ROWS)=F_BOUNDARY+(F(JPT,N_ROWS)-F_BOUNDARY-           &
     &     A_NS(JPT,N_ROWS)**3*(F(JPT,N_ROWS-1)-F_BOUNDARY))            &
