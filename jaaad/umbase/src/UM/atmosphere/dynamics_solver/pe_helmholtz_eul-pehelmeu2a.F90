...
! (1-q)/(1-q_star) - 1 with q terms averaged to rho levels
     ...
     &                     - 1.0
        ...
        Else If ( k  ==  wet_model_levels+1) Then
          Do j = j0-1, j1+1
            Do i = 0, row_length+1
              interp3(i,j) = ( 1.-weight_lower(i,j,k)                   &
     &                         * q(i,j,k-1) ) /                         &
...
! set u + alpha_1 u' correctly since u' is known and held in R_u
        If (model_domain  ==  mt_LAM) Then
          If(at_extremity(PNorth)) Then
            Do i = 1, row_length
              interp6(i,rows) = u(i,rows,k)+ alpha_1 *  R_u(i,rows,k)
            End Do
          End If
          If(at_extremity(PSouth)) Then
            Do i = 1, row_length
              interp6(i,1) = u(i,1,k)+ alpha_1 *  R_u(i,1,k)
            End Do
          End If
          If(at_extremity(PWest)) Then
            Do j = j0 ,j1
              interp6(1,j) = u(1,j,k)+ alpha_1 *  R_u(1,j,k)
            End Do
          End If
          If(at_extremity(PEast)) Then
            Do j = j0 ,j1
              interp6(row_length,j) = u(row_length,j,k)                 &
     &                               + alpha_1 * R_u(row_length,j,k)
              interp6(row_length-1,j) = u(row_length-1,j,k)             &
