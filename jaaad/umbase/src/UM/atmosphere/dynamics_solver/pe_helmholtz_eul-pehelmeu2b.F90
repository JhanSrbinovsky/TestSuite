...
!   6.2   25/12/05  Change loop bound variable names    Terry Davies
!   6.2   25/12/05  Change size of LAM solver domain    Terry Davies
...
! set u + alpha_1 u' since u' is known at boundary and is held in R_u
     ...
     &                                      alpha_1 * R_u(i,j_begin-1,k)
      ...
      &                                    alpha_1 * R_u(i_start-1,j,k)
          ...
          If(at_extremity(PEast)) Then
            Do j = j_start, j_stop
              interp6(i_stop_u+1,j) = u(i_stop_u+1,j,k) +               &
...
! zero u derivative at pole
     ...
     &                   Fv(i,j,k) * temp2
      ...
      END SUBROUTINE PE_Helmholtz_eul
