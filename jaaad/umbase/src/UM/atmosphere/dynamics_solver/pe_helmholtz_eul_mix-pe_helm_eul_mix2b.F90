...
! ----     -------     -------
!  5.4   15/09/01  This deck intoduced (based on pe_helmholtz_eul)
!                                                            A. Malcolm
!  5.5   26/03/03  put swapbounds calls before fill_halos     A.Malcolm
!  5.5   25/02/03  extra moisture variable changes    Andy Malcolm
!  6.1   27/05/04  catchup with changes made to pe_helmholtz  A.Malcolm
!  6.1   02/08/04  Iteration count analysis options       Terry Davies
!  6.1   02/08/04  Remove L_vector since always false     Terry Davies
!  6.2   24/11/05  Changes for cycling semi-Lagrangian scheme.
!                                                        M. Diamantakis
!  6.2   25/12/05  Include option to print l2norms of coefficients
!                                                         Terry Davies
...
! if rims_to_do > 1 (no problem for global configurations)      
        ...
        if (at_extremity(PSouth)) then
          ...
          j_begin = rims_to_do + 1
       endif ! at_extremity(PSouth)
        if (at_extremity(PNorth)) then
          j_stop = rows - rims_to_do
          j_end = rows - rims_to_do
          j_stop_v = n_rows - rims_to_do
        endif ! at_extremity(PNorth)
        ...
        if (at_extremity(PEast)) then
          ...
          i_stop_u = row_length - rims_to_do - 1
...
! Zero weights at LAM boundary points
        ...
        If ( at_extremity(PEast) ) Then
          ...
          End Do
          Do j = j_start - 2, j_stop + 1
            v_mask(i_stop+1,j) = 0.0
          End Do
        End If ! at_extremity(PEast)

      End If ! (.not. L_lbc_new) .and. model_domain == mt_LAM
...
! has intentionally not been replaced as it should not change.
     ...
     &            (1.+mix_v_np1(i,j,k)/epsilon)/(1.+moist_np1(i,j,k))
              thetav_star_rho(i,j,k) = thetav_star(i,j,k)
            End Do
          End Do
     ...
     &                                 thetav_star(i,j,k-1)
      ...
      End If  !  L_new_tdisc .and. CycleNo > 1
...
! calculate G and K terms
     ...
     &                                r_theta_levels(i,j,k-1) )
       ...
       Else  !  CycleNo > 1
        ...
        Do j = 1, rows
          Do i = 1, row_length
            dtheta_dr_term(i,j,k) = ( theta_np1(i,j,k+1) -              &
     &                                theta_np1(i,j,k) ) /              &
     &                              ( r_theta_levels(i,j,k+1) -         &
     &                                r_theta_levels(i,j,k) )
          End Do
...
!  dtheta_dr_term if fully-interpolating SL theta advection is used
...
! set whole domain values (needed to calculate or set u_star)
     ...
     &            f3_at_u(i,j) * f3_at_u(i,j)
        ...
        Else !  variable resolution
         ...
         Do j = j_start-1, j_stop+1
           Do i = i_start - 1, i_stop + 1
            temp1 = v_mask(i,j-1) * recip_1p_cd_phi(i,j-1,k)
            temp2 = v_mask(i,j) * recip_1p_cd_phi(i,j,k)
            wk_temp(i,j) = wt_phi_v(i,j) * temp1 +                      &
     &                     (1.0 - wt_phi_v(i,j)) * temp2
           End Do

           Do i = i_start - 1, i_stop 
            temp1 = wt_lambda_p(i+1) * wk_temp(i,j) +                   &
     &              (1.0 - wt_lambda_p(i+1)) * wk_temp(i+1,j)
            Ju = 1. / recip_1p_cd_lambda(i,j,k) + temp3 * temp1 *       &
     &                f3_at_u(i,j) * f3_at_u(i,j)
            Au(i,j,k) = 1. / Ju
            Fu(i,j,k) = alpha_3 * timestep * f3_at_u(i,j) / Ju
          End Do
         End Do  !  j = j_start, j_stop
...
! set whole domain values (needed to calculate or set v_star)
     ...
     &              f3_at_v(i,j) * f3_at_v(i,j)
        ...
        Else !  variable resolution
          ...
          Do j = j_start - 1, j_stop + 1
           Do i = i_start - 1, i_stop + 1 
              temp1 = u_mask(i-1,j) * recip_1p_cd_lambda(i-1,j,k)
              temp2 = u_mask(i,j) * recip_1p_cd_lambda(i,j,k)
              wk_temp(i,j) =  wt_lambda_u(i) * temp1 +                  &
     &                        (1.0 - wt_lambda_u(i)) * temp2
            End Do
          End Do
...
! set whole domain values (needed to calculate or set v_star)
          Do j = j_start - 1, j_stop
           Do i = i_start - 1, i_stop + 1 
            temp1 = wt_phi_p(i,j+1) * wk_temp(i,j) +                    &
     &              (1.0 - wt_phi_p(i,j+1)) * wk_temp(i,j+1)
            Jv = 1. / recip_1p_cd_phi(i,j,k) + temp4 * temp1 *          &
     &                f3_at_v(i,j) * f3_at_v(i,j)
            Av(i,j,k) = 1. / Jv
            Fv(i,j,k) = alpha_3 * timestep * f3_at_v(i,j) / Jv
...
! (m_star-m)/(1+m_star)(1+m)  with m terms averaged to rho levels
     ...
     &         ( (1. + moist_star(i,j,k)) * (1. + moist(i,j,k)) )
          ...
          Do j = j_start, j_stop
            Do i = i_start, i_stop
              interp3(i,j) = ( weight_upper(i,j,k) *                    &
     &                        (moist_star(i,j,k) - moist(i,j,k)) +      &
     &                         weight_lower(i,j,k) *                    &
     &                       (moist_star(i,j,k-1) - moist(i,j,k-1)) ) / &
     ...
     &                        weight_lower(i,j,k) * moist(i,j,k-1) ) ) )
        ...
        Else If ( k == wet_model_levels+1 ) Then
          ...
          Do j = j_start, j_stop
            Do i = i_start, i_stop
              interp3(i,j) = ( weight_lower(i,j,k) *                    &
     &                       (moist_star(i,j,k-1) - moist(i,j,k-1)) ) / &
     ...
     &          (1. + ( weight_lower(i,j,k) * moist(i,j,k-1) ) ) )
        ...
        Do j = j_start, j_stop
          Do i = i_start, i_stop
            HM_RHS(i,j,k) = HM_RHS(i,j,k) - dr_deta(i,j,k) *            &
     &                      rho(i,j,k) * recip_timestep * interp3(i,j)
          End Do
        End Do
...
! Initialise u_star to zero at poles
      ...
      If(at_extremity(PSouth))then
        Do i = 0, row_length + 1
          u_star(i,rims_to_do) = 0.0
        End Do
      End If
      If(at_extremity(PNorth))then
        Do i = 0, row_length + 1
          u_star(i,rows - rims_to_do + 1) = 0.0
        End Do
      End If
      Endif
...
! Form u_star
     ...
     &                   + R_v(i,j-1,k) * recip_1p_cd_phi(i,j-1,k) ))
        ...
        Else !  variable resolution
          ...
          Do j = j_begin, j_end
            Do i = i_start-1, i_stop + 1
              wk_temp(i,j) = wt_phi_v(i,j) * R_v(i,j-1,k) *             &
     &                       recip_1p_cd_phi(i,j-1,k) +                 &
     &                       (1.0 - wt_phi_v(i,j)) * R_v(i,j,k) *       &
     &                       recip_1p_cd_phi(i,j,k)
         ...
         Do j = j_begin, j_end
          Do i = i_start-1, i_stop
            u_star(i,j) = u(i,j,k) + alpha_1 *                          &
     &                        ( Au(i,j,k) * R_u(i,j,k) + Fu(i,j,k) *    &
     &                              ( wt_lambda_p(i+1) * wk_temp(i,j) + &
     &                    (1.0 - wt_lambda_p(i+1)) * wk_temp(i+1,j) ) )
            End Do
          End Do
        ...
        If ( (.not. L_lbc_new) .and. model_domain == mt_LAM ) then
! Original lbcs set u + alpha_1 u' 
! since u' is known at boundary and is held in R_u

          If(at_extremity(PNorth)) Then
            Do i = i_start-1, i_stop
              u_star(i,j_end+1) = u(i,j_end+1,k) +                      &
     &                             alpha_1 * R_u(i,j_end+1,k)
            End Do
          End If
          If(at_extremity(PSouth)) Then
            Do i = i_start-1, i_stop
              u_star(i,j_begin-1) = u(i,j_begin-1,k) +                  &
     &                                      alpha_1 * R_u(i,j_begin-1,k)
            End Do
          End If
          If(at_extremity(PWest)) Then
            Do j = j_start, j_stop
              u_star(i_start-1,j) = u(i_start-1,j,k) +                  &
      &                                    alpha_1 * R_u(i_start-1,j,k)
            End Do
          End If
          If(at_extremity(PEast)) Then
            Do j = j_start, j_stop
              u_star(i_stop_u+1,j) = u(i_stop_u+1,j,k) +                &
     &                                  alpha_1 * R_u(i_stop_u+1,j,k)
            End Do
          End If
        ...
        Else !  variable resolution
         ...
         Do j = j_begin, j_end
           Do i = i_start, i_stop
            HM_RHS(i,j,k) = HM_RHS(i,j,k) + sec_theta_latitude(i,j) *   &
     &                                             recip_dlamu(i-1) *   &
     &                   (interp3(i,j) * u_star(i,j) * interp2(i,j) -   &
     &                  interp3(i-1,j) * u_star(i-1,j) * interp2(i-1,j))
          End Do
        End Do
...
! Form v_star   store in HM_Cz
     ...
     &               + R_u(i,j,k)* recip_1p_cd_lambda(i,j,k)) )
        ...
        Do j = j_start-1, j_stop + 1
          Do i = i_start, i_stop
            wk_temp(i,j) = wt_lambda_u(i) * R_u(i-1,j,k) *              &
     ...
     &                 (1.0 - wt_phi_p(i,j+1)) * wk_temp(i,j+1) ) )
        ...
        If ( (.not. L_lbc_new) .and. model_domain == mt_LAM ) then
! Original lbcs set v + alpha_1 v' 
!               since v' is known and held in R_v
...
! sum terms at poles and add on
     ...
     &                      model_levels, temp_s, proc_row_group,       &
     &                      info, sum_s)
#else
...
#if defined(REPROD)
          Call gcg_rvecsumr(row_length, row_length, 1,                  &
     &                      model_levels, temp_n, proc_row_group,       &
     &                      info, sum_n)
#else
          Call gcg_rvecsumf(row_length, row_length, 1,                  &
     &                      model_levels, temp_n, proc_row_group,       &
...
! re-scale HM_cyx2 and HM_cxy2 to incorporate friction in solver
     ...
     &                       * recip_1p_cd_phi(i,j,k)
      ...
      If(at_extremity(PNorth))then
        ...
        End Do
      End If
      Endif  !  model_domain  /=  mt_bi_cyclic_lam
...
! Exner_prime at the boundary has been applied by modifying the RHS.
...
! DEPENDS ON: print_l2nhelm
     ...
     &                     .false., .false., rims_to_do,                &
     &                     L_print_pe )
      end if   ! norm_lev_start == norm_lev_end
...
! DEPENDS ON: gcr_k
     ...
     &                 )
      ...
      If (model_domain == mt_LAM .and. L_lbc_new) Then
...
! for use in updating R_u and R_v
        If(at_extremity(PSouth))then
          Do k = 1, model_levels
            do i = 1, row_length
              exner_prime(i,rims_to_do,k) = solver_lbc(i,rims_to_do,k)
            End Do
          End Do !  k = 1, model_levels
        End If
        If(at_extremity(PNorth))then
        ...
        If(at_extremity(PWest))then
          Do k = 1, model_levels
            do j = 1, rows
              exner_prime(rims_to_do,j,k) = solver_lbc(rims_to_do,j,k)
            End Do
          End Do !  k = 1, model_levels
        End If
        If(at_extremity(PEast))then
          Do k = 1, model_levels
            i = row_length - rims_to_do + 1
            do j = 1, rows
              exner_prime(i,j,k) = solver_lbc(i,j,k)
            End Do
          End Do !  k = 1, model_levels
      ...
      EndIf !  model_domain == mt_LAM .and. L_lbc_new
...
! form average vertical pressure derivative
     ...
     &                           * HM_Cxy2(i,j,k)
            ...
            Do j = j_begin, j_end + 1
              Do i = i_start - 1, i_stop
                temp1 = wt_lambda_p(i+1) * interp1(i,j) +               &
     &                   (1.0 - wt_lambda_p(i+1)) * interp1(i+1,j)
                temp2 = (exner_prime(i+1,j,k) - exner_prime(i,j,k)) *   &
     &                                          recip_dlamp(i)
                interp5(i,j) = (temp2 - HM_Cxp(i,j,k) * temp1) *        &
     &                                  HM_Cyx2(i,j,k)
              End Do
            End Do
            ...
            Do j = j_begin - 1, j_end
              Do i = i_start, i_stop_u + 1
                temp1 =  wt_phi_p(i,j+1) * interp1(i,j) +               &
     &                   (1.0 - wt_phi_p(i,j+1)) * interp1(i,j+1)
                temp2 = (exner_prime(i,j+1,k) - exner_prime(i,j,k)) *   &
     &                                          recip_dphip(i,j)
                interp6(i,j) = (temp2 - HM_Cyp(i,j,k) * temp1) *        &
     &                                 HM_Cxy2(i,j,k)
              End Do
            End Do
...
! Final calculation of v_prime is after u increment calculation.
     ...
     &                             recip_1p_cd_lambda(i,j,k))
        ...
        Else ! variable resolution
          Do j = j_start, j_stop_v + 1
            Do i = i_start, i_stop
              interp2(i,j) = wt_lambda_u(i) * R_u(i-1,j,k) *            &
     &                          recip_1p_cd_lambda(i-1,j,k) +           &
     &                              (1.0 - wt_lambda_u(i)) *            &
     &                  R_u(i,j,k) * recip_1p_cd_lambda(i,j,k)
              interp1(i,j) = wt_lambda_u(i) * interp5(i-1,j) *          &
     &                            recip_1p_cd_lambda(i-1,j,k) +         &
     &                                (1.0 - wt_lambda_u(i)) *          &
     &                  interp5(i,j) * recip_1p_cd_lambda(i,j,k)
            End Do
          End Do
        End If ! L_regular
...
! (not updated on LAM boundaries)
     ...
     &                  )
        ...
        Do j = j_begin, j_end
          Do i = i_start, i_stop_u + 1
            temp3 = ( R_v(i,j-1,k) - temp1 *interp6(i,j-1) )            &
     &                              * recip_1p_cd_phi(i,j-1,k)
            temp2 = ( R_v(i,j,k) - temp1 *interp6(i,j) )                &
     &                              * recip_1p_cd_phi(i,j,k)
            wk_temp(i,j) = wt_phi_v(i,j) * temp3 +                      &
     &                     (1.0 - wt_phi_v(i,j)) * temp2
          End Do
        End Do
        ...
        Do j = j_begin, j_end
          Do i = i_start, i_stop_u
            temp2 = wt_lambda_p(i+1) * wk_temp(i,j) +                   &
     &               (1.0 - wt_lambda_p(i+1)) * wk_temp(i+1,j)
            R_u(i,j,k) = Au(i,j,k) *                                    &
     &                  ( R_u(i,j,k) - temp1 * interp5(i,j)) +          &
     &                   Fu(i,j,k) * temp2
...
! zero u derivative at pole
     ...
     &                           (interp1(i,j) + interp1(i,j+1) ) )
        ...
        Do j = j_start, j_stop_v
          Do i = i_start, i_stop
            temp2 = wt_phi_p(i,j+1) *                                   &
     &              (interp2(i,j) - temp1 * interp1(i,j)) +             &
     &              (1.0 - wt_phi_p(i,j+1)) *                           &
     &              (interp2(i,j+1) - temp1 * interp1(i,j+1))
            R_v(i,j,k) = Av(i,j,k) *                                    &
     &                   ( R_v(i,j,k) - temp1 *interp6(i,j) ) -         &
     &                   Fv(i,j,k) * temp2
          End Do
      ...
      Return   ! End of routine PE_Helmholtz_eul
