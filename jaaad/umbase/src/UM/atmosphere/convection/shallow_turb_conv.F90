...
! Stash flags
      ...
      Logical, intent(in) ::                                            &
     &  flg_uw_shall                                                    &
                      ! STASH flag for x-comp shallow stress
     &, flg_vw_shall  ! STASH flag for y-comp shallow stress
...
!-----------------------------------------------------------------------
      ...
      Character*(*), Parameter ::  RoutineName = 'shallow_turb_conv'
...
!
! Constants evaluated once to save CPU
!
      Real ::                                                           &
     & lc_o_cp                                                          &
                    ! Lc/Cp
...
!
      EXTERNAL                                                          &
     &  shconv_cloudbase, shconv_inversion                              &
     &, les_shall_func_thlev, les_shall_func_rhlev                      &
     &, shconv_qlup, shallow_wql, shallow_grad_h                        &
     &, shconv_turb_fluxes, sh_grad_flux , shconv_precip                &
     &, shtconv_grad_stress,shtconv_base_stress,shtconv_cmt_incr        
#if defined(SCMA)
      EXTERNAL                                                          &
     & SCMoutput
#endif
...
!
     ...
     &      (1.+q_mix(i,ntpar(i)+1)/epsilon)/(1.+q_mix(i,ntpar(i)+1))   &
     &                  -   theta(i,ntml(i))*                           &
     &      (1.+q_mix(i,ntml(i))/epsilon)/(1.+q_mix(i,ntml(i)))
...
!
        ...
        temp2=-gamma_dry*(1.+t_lcl_temp*temp1*r_epsilon)                &
     &                                /(1.+lc_o_cp*temp1)
...
! Note fill arrays even if above cloud top
      ...
      Do k=1,max_cldlev+1
        Do i = 1,n_sh
          ...
          thetav_cld(i,k) = theta(i,klev)*(1+q_mix(i,klev)/epsilon)     &
     &                             /(1.+q_mix(i,klev))
...
! conservation check -left for SCM as a means of checking code
! Note conservation is not exact 
        ...
        Do ktra = 1,ntra
          ...
          Do k = 1,trlev
            Do i = 1,n_sh
              mtracer(i,ktra) = mtracer(i,ktra)+ dtrabydt(i,k,ktra)     &
     &                              *r2rho_th(i,k)*dr_across_th(i,k)    &
     &                              /(earth_radius*earth_radius)
              ttracer(i,ktra) = ttracer(i,ktra)+ tracer(i,k,ktra)       &
     &                              *r2rho_th(i,k)*dr_across_th(i,k)    &
     &                              /(earth_radius*earth_radius)
            End Do
          End Do
     ...
     &                              /(earth_radius*earth_radius)
          End Do
        End Do
      Write(6,*) 'tracer conservation increments'
      Write(6,*) (mtracer(1,ktra),ktra=1,ntra)
      Write(6,*) (ttracer(1,ktra),ktra=1,ntra)
      Write(6,*) 'mass total column ',masscol(1)
...
! DEPENDS ON: scmoutput
      ...
      rout=wql_cb
      sname='wql_cb'
      lname='wql at plus side of cloud base'
      units='Kg/kg m/s'
! DEPENDS ON: scmoutput
      Call SCMoutput(rout,sname,lname,units,t_inst,d_point              &
         , default_streams,'',RoutineName)
...
! DEPENDS ON: scmoutput
      ...
      Do i=1,n_sh
      rout=theta_plus(i) *                                              &
     &                        (1.+q_plus(i)*r_epsilon)/(1.+q_plus(i))   &
     &    -theta_minus(i)*                                              &
     &                        (1.+q_minus(i)*r_epsilon)/(1.+q_minus(i))
      End do
      sname='dthetav_cb'
...
! DEPENDS ON: scmoutput
      ...
      Do i=1,n_sh
        rout=wthetal_cb(i)+c_virtual*theta_minus(i)*wqt_cb(i)
      End do
      sname='wthetav_m_cb'
...
! zero top level of output array
      k=nlev+1
      Do i=1,n_sh
        r2out(i,k)= 0.0
      End Do
