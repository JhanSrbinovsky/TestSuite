...
! -------     ----     -------
!
!   6.2    20/07/04   New code    R A Stratton
!   6.4    05/04/06   Add precipitation. R A Stratton
!   6.4    03/08/06   Add tracers. R A Stratton
!   6.4    20/09/06   Revised Inversion calculation using another level
!                    (in code but switched off as unsafe).  Corrected
!                     div_term2 to use 1+c_virtual rather than c_virtual.
!                     R A Stratton
!
! Code Description:
...
! For mixing ratio
     ...
     &                        (1.+r_below(i)/epsilon)/(1.+r_below(i))
        thetav_mid(i)   = theta_mid(i)*                                 &
     &                        (1.+r_mid(i)/epsilon)/(1.+r_mid(i))
        thetav_above(i) = theta_above(i)*                               &
     &                        (1.+r_above(i)/epsilon)/(1.+r_above(i))
...
!-----------------------------------------------------------------------
      ...
      If (l_3lev_inv) then
       Do i = 1,n_sh
        If (r_mid(i) >  0.0) then
          vap_press = 0.01*r_mid(i)*p_mid(i)/(epsilon+r_mid(i))
        Else
          vap_press = 0.01*r_min*p_mid(i)/(epsilon+r_min)
        End If
        t_lcl_mid(i) =  a_bolton + b_bolton / ( c_bolton*log(t_mid(i))  &
     &                                    - log(vap_press) - d_bolton )
        pstar_mid(i) = p_mid(i) *                                       &
     &                     ( t_lcl_mid(i) / t_mid(i) )**(1.0/KAPPA)

        If (r_below(i) >  0.0) then
          vap_press = 0.01*r_below(i)*p_below(i)/(epsilon+r_below(i))
        Else
          vap_press = 0.01*r_min*p_below(i)/(epsilon+r_min)
        End If
        t_lcl_below(i) = a_bolton + b_bolton/( c_bolton*log(t_below(i)) &
     &                                    - log(vap_press) - d_bolton )
        pstar_below(i) = p_below(i) *                                   &
     &                   ( t_lcl_below(i) / t_below(i) )**(1.0/KAPPA)

        If (r_above(i) >  0.0) then
          vap_press = 0.01*r_above(i)*p_above(i)/(epsilon+r_above(i))
        Else
          vap_press = 0.01*r_min*p_above(i)/(epsilon+r_min)
        End If
        t_lcl_above(i) = a_bolton + b_bolton/( c_bolton*log(t_above(i)) &
     &                                    - log(vap_press) - d_bolton )
        pstar_above(i) = p_above(i) *                                   &
     &                   ( t_lcl_above(i) / t_above(i) )**(1.0/KAPPA)
       End do
