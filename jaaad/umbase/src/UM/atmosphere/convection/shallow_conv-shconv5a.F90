...
!
     ...
     &                       wstar,wthvs,zlcl_uv,                       &
     &                       ztop_uv,freeze_lev,recip_pstar,qse,        &
...
!
      Use cv_cntl_mod, Only:                                            &
          lcv_ccrad, lcv_3d_cca, lcv_pc2_diag_sh
...
!     &, r_rho(n_sh,nlev)     ! radius of rho levels (m)
!     &, r_theta(n_sh,0:nlev) ! radius of theta levels + surface(m)
!     &, rho(n_sh,nlev)       ! density on rho levels kg/m3
...
!
      ...
      real, intent(out) :: cca(n_sh,nlev) ! Convective cloud amount on model
                                          ! levels (fraction)
...
!
      ...
      real, intent(out) :: the_out(n_sh,nlev)    !th_E in parcel routine

      real, intent(out) :: thp_out(n_sh,nlev)    !th_P in parcel routine
...
! PC2 compression arrays
      ...
      real   :: zpr         ! method (BL Fluxes) only if
                            !   lcv_ccrad = T .AND. cca2d_sh_opt  = 1
...
!      call Cor_engy(n_sh,n_sh,n_sh,nlev,dthbydt,dqbydt,snow
!     &               ,exner_layer_centres,p_layer_boundaries,index1)

!      call Cor_engy(n_sh,n_sh,n_sh,nlev,index1,.false.,r2rho_th
...
!-------------------------------------------------------------------------
      ...
      If (lcv_ccrad) Then
        ...
        Do i=1, n_sh
          ...
          !---------------------------------------
          cca_2d(i) = cca_sh_knob*cca_2d(i)

          !---------------------------------------
          ! Make sure cca_2d remains in limits
          !---------------------------------------
      ...
      Else        ! Non CCRAD option
       ...
       ! Note: if (l_q_interact) then cca remains set to zero.
       ! If lcv_pc2_diag_sh   - diagnostics shallow cloud  then non-zero 

        If (.not.l_q_interact.or.lcv_pc2_diag_sh) Then
          Do k = 1,n_cca_lev
            Do i = 1,n_sh
