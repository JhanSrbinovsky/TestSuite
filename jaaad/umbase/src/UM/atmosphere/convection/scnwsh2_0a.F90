...
SUBROUTINE scnwsh2(row_length, rows, off_x, off_y, halo_i, halo_j, model_levels &
         , wet_model_levels, r_rho_levels, r_theta_levels, timestep             &
         , substep_number, rho, q, qcl, qcf, s_mmr, ccldbase, ccldtop, rainrate &
         , accu_scav_tr                                                         &
         )

!-------------------------------------------------------------------------------
! Description:
!   Dummy stub of SCNWSH2 which is called if Convection Scheme is disabled.
!
...
!----------
  ...
  REAL, INTENT(IN) :: s_mmr          (1-off_x  : row_length+off_x,              &
                                      1-off_y  : rows      +off_y,              &
                                                 model_levels)
                                               ! mmr S in SO2
...
!-------------------------------------------------------------------------------
  ...
  WRITE (6,*) '**ERROR**: SCNWSH2 called but is unavailable.    '
