...
! uncompressed arrays - all points
      ...
      real ::                      &
     & qs_star(row_length, rows)   & ! Saturated sp humidity at sea surface
...
!                                ! buoyancy below 3km (for shallow Cu)
     ...
     &,P_LCL(row_length*rows)           & ! Pressure at lifting condensation
                                          ! level.
     &,exner_LCL(row_length*rows)       & ! Exner at lifting condensation
                                          ! level.
     &,exner_surf(row_length*rows)      & ! Exner at surface
...
! capped by an inversion.
     ...
     &,ql_ad2(row_length*rows)            ! ql_ad 2nd copy
...
!
     ...
     &, dtdz            &  ! dT/dz parcel temp gradient
     &, dqsatdz         &  ! dqsat/dz parcel
...
!-----------------------------------------------------------------------
! 3.1 Calculate T at old time level.
!-----------------------------------------------------------------------
...
! expression for mixing ratio
     ...
     &                ( T_LCL(ii) / T(ii,k_plume(ii)) )**(1.0/kappa)
          ...
          Else
           ...
           P_LCL(ii) = pstar(i,j)
          End If
          exner_lcl(ii) = (p_lcl(ii)/pref)**kappa
...
! required for ql_ad calculation
! DEPENDS ON: qsat_mix
        Call qsat_mix(qsat_lev,T_parc(1,k),p_theta_lev_c(1,k)          &
     &                                   ,nunstable,l_mixing_ratio)
...
!-----------------------------------------------------------------------
          ...
          If (K > nlcl_c(ii) .and. k < wet_model_levels ) then
            ...
            dtdz = (T_parc(ii,k) - T_parc(ii,k-1))/                    &
     &                     ( z_full_c(ii,K) - z_full_c(ii,K-1) )

            dqsatdz = (qsat_lev(ii)/(R*T_parc(ii,k)))                  &
     &              * (dtdz*epsilon*l_const/T_parc(ii,k) + g) 
...
!-----------------------------------------------------------------------
! altered for 5A to ensure nptar -ntml >=3 (at this point ntml holds ntpar)
...
!-----------------------------------------------------------------------
! Is a congestus diagnosis required ?
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
           If ( shmin(ii) ) then
             ...
             If ( (z_full_c(ii,kshmin(ii)) - z_full_c(ii,ntpar(i,j)))  &
...
! No check on descending air 
              ...
              If ( (T(ii,ntpar(i,j))  >=  TM) .and.                    &
     &          (z_full_c(ii,ntpar(i,j)) >  2500.)) then
                l_congestus(i,j) = .true.
              Else

! Top of convection below 2500km

                If (z_full_c(ii,ntpar(i,j)) <= 2500.) then
                  If  (w_avg(ii)  <   0.0) then
! Air descending
                     L_shallow(i,j) = .TRUE.
                  Else

! Air rising therefore class as congestus  rather than deep
                     l_congestus(i,j) = .true.
                  End If    ! w_avg test 
...
!-----------------------------------------------------------------------
             nlcl(i,j) = nlcl(i,j)+1
             z_lcl_nlcl(i,j) = z_half_c(ii,nlcl(i,j)+1)
             z_lcl_uv(i,j)= z_full_c(ii,nlcl(i,j))
           End If
           zh(i,j) = z_lcl_nlcl(i,j)
           ntml(i,j) = nlcl(i,j)
...
! required for ql_ad calculation  from dilute or undilute ?
! Currently dilute value ?
! DEPENDS ON: qsat_mix
        Call qsat_mix(qsat_lev_dil,T_parc_dil(1,k),p_theta_lev_c(1,k)  &
     &                                      ,nunstable,l_mixing_ratio)
...
! calculation of CIN and CAPE from profiles - undilute values 
     ...
     &          * (z_half_c(ii,K+1) - z_half_c(ii,K))/t_dens_env(ii,k)
              ...
              dtdz = (T_parc_dil(ii,k) - T_parc_dil(ii,k-1))/          &
     &                     ( z_full_c(ii,K) - z_full_c(ii,K-1) )
              If (T_parc_dil(ii,k).gt.TM) then
                l_const=lc
              Else
                l_const=lc+lf
              End If
              dqsatdz = qsat_lev(ii)*                                  &
     &                     (dtdz*epsilon*l_const/T_parc_dil(ii,k)+g)   &
     &                           /(R*T_parc_dil(ii,k))  
...
!-----------------------------------------------------------------------
! Is a congestus diagnosis required as well as shallow ?
!-----------------------------------------------------------------------
      If (iconv_congestus >  0) then
...
! No check on descending air 
            ...
            If ( (T(ii,ntpar(i,j))  >=  TM) .and.                      & 
                        z_full_c(ii,ntpar(i,j))  > 2500.0 ) then

                l_congestus(i,j) = .true.

            Else if ( z_full_c(ii,ntpar(i,j))  <= 2500.0 ) then

! Top of convection below 2500km

              If (l_wtest) then
! Only shallow if descending air - if w test to be applied
...
!
!  l_congestus  - all congestus points
!  l_congestus2 - congestus points with descending air above
...
!      If cumulus has been diagnosed but delthvu is negative, reset
!      cumulus, l_shallow, l_congestus to FALSE but leave zh and ntml 
!      at LCL
...
!-----------------------------------------------------------------------
! Original shallow diagnosis no congestus diagnosed
!-----------------------------------------------------------------------
!CDIR NODEP
        Do ii=1,nunstable
          i = index_i(ii) 
...
!-----------------------------------------------------------------------
! Accurate calculation of zlcl using exner_lcl rather than p_lcl as 
! UM interpolates exner linearly in height but not pressure.
!-----------------------------------------------------------------------
      Do ii=1, nunstable
        i = index_i(ii)   
        j = index_j(ii)   
        exner_surf(ii) = (pstar(i,j)/pref)**kappa     
      End Do
      ...
      Do k=1,model_levels
        ...
        Do ii=1, nunstable
          ...
          j = index_j(ii)   
          If (k == 1) then 
            If ( exner_lcl(ii) >= exner_surf(ii)) then 
              z_lcl(i,j) = 0.0           ! at or below surface
            Else If (exner_lcl(ii) < exner_surf(ii)                     &
                           .and. exner_lcl(ii) > exner_rho(i,j,k)) Then
              factor= (exner_rho(i,j,k) - exner_lcl(ii))/               &
                        (exner_rho(i,j,k) - exner_surf(ii))
              z_lcl(i,j) = (1.0-factor)*z_half(i,j,k)  
            End If
          Else
            If (exner_lcl(ii) >= exner_rho(i,j,k)                       &
                        .and. exner_lcl(ii) < exner_rho(i,j,k-1) ) Then    
              factor= (exner_rho(i,j,k) - exner_lcl(ii))/               &
                        (exner_rho(i,j,k) - exner_rho(i,j,k-1))
              z_lcl(i,j) = (1.0-factor)*z_half(i,j,k)+factor*z_half(i,j,k-1)
            End If   
          End If       ! test on k=1
          
        End Do         ! unstable points
      End Do           ! level loop

      Endif ! test on nunstable
...
! DEPENDS ON: scmoutput
        Call SCMoutput(z_lcl_nlcl,                                     &
             'z_lcl_nlcl',' LCL height at nlcl ','m',                           &
             t_inst,d_point,default_streams,'',RoutineName)
