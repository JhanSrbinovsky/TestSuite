...
! ######################################################################
!
      SUBROUTINE ukca_transform_halogen(tr_vars, rows, row_length,      &
                              model_levels,                             &
                              off_x, off_y, tracers, halo_i, halo_j,    &
                              q, forward, timestep)

! This subroutine changes from representations of total chlorine in HCl
! and frations of total chlorine in the other tracers, to proper
...
! Subroutine interface
      ...
      REAL, INTENT(INOUT) :: tracers(1-off_x:row_length+off_x,          &
                     1-off_y:rows+off_y, model_levels, tr_vars)
        ...
        DO i=1,jpctr
          SELECT CASE (advt(i))
            ...
            CASE('MeCl      ')
              ...
              s_cl_tracers(n_cl_tracers) = source_gas
              n_h_tracers = n_h_tracers + 1
              c_h_tracers(n_h_tracers) = c_mecl
              h_tracers(n_h_tracers) = i
              s_h_tracers(n_h_tracers) = mixed_compound
            CASE('CCl4      ')
              n_cl_tracers = n_cl_tracers + 1
            ...
            CASE('MeCCl3    ')
              ...
              s_cl_tracers(n_cl_tracers) = source_gas
              n_h_tracers = n_h_tracers + 1
              c_h_tracers(n_h_tracers) = c_meccl3
              h_tracers(n_h_tracers) = i
              s_h_tracers(n_h_tracers) = mixed_compound
            CASE('BrCl      ')
              n_cl_tracers = n_cl_tracers + 1
...
! bromine tracers
            ...
            CASE('MeBr      ')
              ...
              n_h_tracers = n_h_tracers + 1
              c_h_tracers(n_h_tracers) = c_mebr
              h_tracers(n_h_tracers) = i
              s_h_tracers(n_h_tracers) = mixed_compound
            CASE('CF3Br     ')
              n_br_tracers = n_br_tracers + 1
...
! DEPENDS ON: ereport
              CALL EREPORT('UKCA_TRANSFORM_HALOGEN',1,cmessage)
          END SELECT
        END DO
        first = .FALSE.
      END IF
      IF ((n_h2os > 0) .AND. (timestep == 1)) THEN
        ...
        CALL transform(row_length, rows, model_levels, off_x,           &
                   off_y, tr_vars, tracers, n_h_tracers, h_tracers,     &
      ...
      IF (forward) THEN
        ...
        CALL transform(row_length, rows, model_levels, off_x,           &
                     off_y, tr_vars, tracers, n_n_tracers, n_tracers,   &
                     c_n_tracers, s_n_tracers, n_no2, c_no2,            &
                     forward)

        CALL transform(row_length, rows, model_levels, off_x,           &
                     off_y, tr_vars, tracers, n_cl_tracers, cl_tracers, &
                     c_cl_tracers, s_cl_tracers, n_hcl, c_hcl,          &
                     forward)

        CALL transform(row_length, rows, model_levels, off_x,           &
                     off_y, tr_vars, tracers, n_br_tracers, br_tracers, &
...
!====================================================================
      ...
      SUBROUTINE transform(row_length, rows, model_levels, off_x,       &
                              off_y, tr_vars, tracers, n_tracers,       &
                              position,                                 &
