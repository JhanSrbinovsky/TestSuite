...
! Subroutine Interface:
      SUBROUTINE UKCA_WETOX(NBOX,ND,DELSO2,DELSO2_2,                    &
       DELH2O2,LOWCLOUD,VFAC,SO2VMR,H2O2,ZH2O2,ZO3,ZHO2,PMID,           &
       T,AIRD,S,DTC,LDAY,LWC)
!----------------------------------------------------------------------
!
!     Purpose
!     -------
...
! .. below stores dissolved ion concentrations for H+,HSO3-
          HPLUSM=10.0**(-PHCLOUD)
          HSO3M=K_S1*HSO2*PSO2ATM/HPLUSM
...
!---------------------------------------------------------------------
         ...
         ENDIF ! if some particles in either soluble accum or coarse
        ENDIF ! if either soluble accum or coarse are defined
       ENDIF ! if grid box is in cloud
!
!---------------------------------------------------------------------
