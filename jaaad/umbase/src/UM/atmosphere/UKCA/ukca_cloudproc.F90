...
!  NUM_EPS  : Value of NEWN below which do not recalculate MD (per cc) or carry out process
...
! .. now set new number to mode IMODE+1
           ...
           ENDIF ! IF NEWN>0
!
          ENDIF ! if FRAC_N<0.99 (if more than 1% activated)
         ELSE
          DO ICP=1,NCP
           IF(COMPONENT(IMODE,ICP)) THEN
        ...
        ENDIF ! if F>0 (if in cloud)
       ENDDO ! end loop over boxes
      ENDIF ! if iactmethod=1
