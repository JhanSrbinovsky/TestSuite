...
! 1980/5/18   ,1982/3/28-4/4, 1985/11/13,   1991/5/15, 1991/8/12-8/15 1979/9/1
...
! SPARC aerosol climatology
      REAL, PARAMETER :: height_top(nerupt) =                           &
                      (/22000.,32000.,26000.,28000.,22000.,30000./)
...
! rescale magnitude into kg(S) / timestep
        emission = magnitude / (end_volc - start_volc + 1.)             &
                     * timestep / day_in_secs * c_s/c_so2
...
! check whether in eruptive phase
          IF ((day_of_year >= start_volc(k)) .AND.                      &
            ...
            j = lat_index(k)
            l1 = 1
            DO WHILE (r_theta_levels(i,j,l1) - earth_radius <=          &
              tropopause_height(i,j))
              l1 = l1 + 1
            END DO
            l2 = model_levels
