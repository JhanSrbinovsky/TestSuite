...
! History:
!
!  v1.1            Original code from Peter Braesicke
!  v1.2 18/10/2004 Introduce limitations of rates to prevent instabilities
!                                         Olaf Morgenstern
!  v1.3 05/01/2006 Remove pressure-based cut-off for PSC chemistry. Replace
!                  with tropopause-diagnostic based.
!                                         Olaf Morgenstern
!  v1.4 18/1/2006  Activate reactions on SO4 aerosol
!                                         Olaf Morgenstern
!  v1.5 31/1/2006  Vectorize code, remove bug in recognition of ClONO2+H2O
!                  and N2O5+H2O reactions
!                                         Olaf Morgenstern
!  v1.6 19/06/2006 Minor changes for code-merging
!                                         Olaf Morgenstern
!  v1.7 25/6/2006  Minor changes for FCM  Olaf Morgenstern
! Code Description:
...
! pressure in hPa here.
      ...
      zt(1:n_points)      = MAX(192.0,t(1:n_points))
...
! 1. ClONO2 + H2O --> HOCl + HNO3
      ...
      IF (n_clono2_h2o > 0) THEN
        IF (.NOT.L_ukca_family) THEN
          rk(:,n_clono2_h2o) = hk(:,1)
        ELSE
          WHERE (cdt*MAX(zh2o,zclono2) > limit)
            rk(:,n_clono2_h2o)=MIN(hk(:,1),                             &
                               0.03/(MAX(zh2o,zclono2)*cdt))
          ELSEWHERE
            rk(:,n_clono2_h2o) = 0.
          END WHERE
        END IF
...
! 2. ClONO2 + HCl --> Cl2 + HNO3
        ...
        ELSE
          WHERE (cdt*MAX(zclono2,zhcl) > limit)
            rk(:,n_clono2_hcl)=MIN(hk(:,2),                             &
                             0.03/(MAX(zclono2,zhcl)*cdt))
          ELSEWHERE
            rk(:,n_clono2_hcl) = 0.
          END WHERE
        END IF
...
! 3. HOCl + HCl --> Cl2 + H2O
        IF (.NOT. L_ukca_family) THEN
          rk(:,n_hocl_hcl) = hk(:,3)
        ELSE
          WHERE (cdt*MAX(zhocl,zhcl) > limit)
            rk(:,n_hocl_hcl)=MIN(hk(:,3),                               &
                           0.03/(MAX(zhocl,zhcl)*cdt))
          ELSEWHERE
            rk(:,n_hocl_hcl) = 0.
          END WHERE
        END IF
...
! 4. N2O5 + H2O -> 2 HNO3
        IF (.NOT. L_ukca_family) THEN
          rk(:,n_n2o5_h2o) = hk(:,4)
        ELSE
          WHERE (cdt*MAX(zn2o5,zh2o) > limit)
            rk(:,n_n2o5_h2o)=MIN(hk(:,4),                               &
                           0.03/(MAX(zn2o5,zh2o)*cdt))
          ELSEWHERE
            rk(:,n_n2o5_h2o) = 0.
          END WHERE
        END IF
...
! 5. N2O5 + HCl -> ClNO2 + HNO3
        IF (.NOT. L_ukca_family) THEN
          rk(:,n_n2o5_hcl) = hk(:,5)
        ELSE
          WHERE (cdt*MAX(zn2o5,zhcl) > limit)
            rk(:,n_n2o5_hcl)=MIN(hk(:,5),                               &
                           0.03/(MAX(zn2o5,zhcl)*cdt))
          ELSEWHERE
            rk(:,n_n2o5_hcl) = 0.
          END WHERE
        END IF
...
!
!  v1.1            Original code from Peter Braesicke
!  v1.2 24/11/2005 Cosmetic changes   O Morgenstern
!
! Code Description:
! Language: FORTRAN 90 + common extensions.
!
...
! DEPENDS ON: ereport
        IF (ihno3 == 0)                                                 &
          CALL ereport('SOLIDPHASE',1,                                  &
           'Select HONO2 as advected tracer.')
      END IF
