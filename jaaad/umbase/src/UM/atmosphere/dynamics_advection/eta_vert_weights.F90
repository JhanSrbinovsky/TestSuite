...
#if defined(FUJITSU)
! lines below are fujitsu compiler directives to speed up code
!ocl repeat(512)
!ocl unroll(8)
#endif
      Subroutine eta_vert_weights (                                     &
...
! -------  -------   -------
!  5.2   26/09/00  Original code (do interpolation in eta) Andy Malcolm
...
!  6.0  18/08/03  NEC SX-6 optimisations - loops collapsed and
!                 logical search replaced by list processing.
!                 R Barnes & J-C Rioual.
!  6.2   15/02/05  Protect vectorization optimizations
!                  with(NEC)def            Tom Edwards
!
! Code Description:
...
! loop collapsed
     ...
     &                     + coeff_c *                                  &
     &                    r_in (i_out(i,j,k),j_out(i,j,k)+1,index)      &
     &                     + coeff_d *                                  &
     &                    r_in (i_out(i,j,k)+1,j_out(i,j,k)+1,index)
...
! bottom most levels or top most levels.
     ...
     &                  + coeff_c                                       &
     &         * r_in (i_out(i,j,k),j_out(i,j,k)+1,k_out(i,j,k))        &
     &                  + coeff_d                                       &
     &         * r_in (i_out(i,j,k)+1,j_out(i,j,k)+1,k_out(i,j,k))
     ...
     &                  + coeff_c                                       &
     &        *  r_in (i_out(i,j,k),j_out(i,j,k)+1,k_out(i,j,k)+1)      &
     &                  + coeff_d                                       &
     &        *  r_in (i_out(i,j,k)+1,j_out(i,j,k)+1,k_out(i,j,k)+1)
