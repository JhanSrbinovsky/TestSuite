...
!
     ...
     &                         rims_to_do, Pi, Cp, g, model_domain,     &
...
!VarRes horizontal co-ordinate information etc.
     ...
     &  f1_at_v (row_length, 0:n_rows+1)                                &
     &, f2_at_u (0:row_length+1, rows)                                  &
...
! DEPENDS ON: fill_external_halos
         Call FILL_EXTERNAL_HALOS(Yu(1-halo_i,1-halo_j,1,k),            &
     &                            row_length,rows,model_levels,         &
     &                            halo_i,halo_j)

! DEPENDS ON: fill_external_halos
         Call FILL_EXTERNAL_HALOS(Yv(1-halo_i,1-halo_j,1,k),            &
     ...
     &                            halo_i,halo_j)
      ...
      Do k = 1, n_Yw_arrays
! DEPENDS ON: fill_external_halos
         Call FILL_EXTERNAL_HALOS(Yw(1-halo_i,1-halo_j,0,k),            &
     &                            row_length,rows,model_levels+1,       &
     &                            halo_i,halo_j)
      End Do  !  k = 1, n_Yw_arrays
...
! DEPENDS ON: zero_lateral_boundaries
     ...
     &        Yu(1-halo_i,1-halo_j,1,k), rims_to_do, AT_EXTREMITY,      &
     &        L_do_boundaries, L_do_halos)

! DEPENDS ON: zero_lateral_boundaries
            CALL ZERO_LATERAL_BOUNDARIES(                               &
     &        ROW_LENGTH,N_ROWS,halo_i,halo_j,MODEL_LEVELS,fld_type_v,  &
     &        Yv(1-halo_i,1-halo_j,1,k), rims_to_do, AT_EXTREMITY,      &
     &        L_do_boundaries, L_do_halos)
...
! DEPENDS ON: set_lateral_boundaries
     ...
     &        RIMWIDTH,rims_to_do,rimweights,AT_EXTREMITY,              &
     &        L_do_boundaries, L_do_halos)
...
! DEPENDS ON: zero_lateral_boundaries
     ...
     &        Yw(1-halo_i,1-halo_j,0,1),rims_to_do,AT_EXTREMITY,        &
     &        L_do_boundaries,L_do_halos)
...
!! interpolate from w (theta levels) on to rho levels
     ...
     &             r_rho_levels(i,j,model_levels))then
          ...
          End Do
        End do
        call gc_ssync(n_proc,error_code)
...
! DEPENDS ON: fill_external_halos
        call fill_external_halos(interp_work1,row_length, rows,         &
     &        model_levels, off_x,off_y)

! DEPENDS ON: fill_external_halos
        call fill_external_halos(interp_work2,row_length, rows,         &
     &        model_levels, off_x,off_y)

! DEPENDS ON: fill_external_halos
        call fill_external_halos(interp_work3,row_length, rows,         &
     &        model_levels, off_x,off_y)
...
! DEPENDS ON: sl_vector_u
     ...
     &                  timestep, Pi, alpha_3, alpha_4, LAM_max_cfl,    &
     &                  row_length, rows, n_rows, model_levels,         &
     &                  Depart_scheme, Depart_order,                    &
     &                  high_order_scheme, monotone_scheme,             &
     ...
     &                  l_datastart, at_extremity, g_i_pe,              &
     &                  proc_row_group, proc_col_group,                 &
     &                  L_sl_halo_reprod, L_new_tdisc, CycleNo, R_u,    &
     &                  Error_code)
...
! DEPENDS ON: sl_vector_v
     ...
     &                    l_datastart, at_extremity, g_i_pe,            &
     &                    proc_row_group, proc_col_group,               &
     &                    L_sl_halo_reprod, L_new_tdisc, CycleNo,       &
     &                    R_v, Error_code)
...
! DEPENDS ON: sl_vector_w
     ...
     &                    global_row_length, global_rows,               &
     &                    l_datastart, at_extremity, g_i_pe,            &
     &                    proc_row_group, proc_col_group, L_regular,    &
     &                    L_sl_halo_reprod, L_new_tdisc, CycleNo,       &
     &                    R_w, Error_code)
