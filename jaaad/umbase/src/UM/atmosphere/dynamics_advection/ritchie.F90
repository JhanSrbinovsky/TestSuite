...
!
     ...
     &                    sin_theta_longitude,                          &
     &                    cos_theta_longitude,                          &
     &                    Pi, Depart_order, rows_depart, model_domain,  &
...
!
!
! Original Programmer: M H Mawson
!
! History:
! Date     Version     Comment
! ----     -------     -------
! 22/03/00 5.1         Updated some code for cyclic_LAM configuration.
!                                                        Andy Malcolm
...
! Local Variables.
     ...
     &, interpw_wind3 (row_length, rows_depart, model_levels)           &
                                                              ! olated.
     &, interpw_wind (0:row_length+1, 0:rows_depart)                    &
     &, lambda_a(row_length)                                            &
...
! External Routines:
! subroutines
      External Polar_Vector_Wind_n
      External Interpolation
      External Calc_Index
      External Bi_Linear_H, check_sl_domain
! Functions: None
...
! convert back to the old co-ordinate system
     ...
     &                          + Atan2(exp1,exp2)
                ...
                depart_phi(i,j,k)=  Asin(cos_depart_phi*                &
     &                        cos_depart_lambda                         &
...
! convert back to the old co-ordinate system
     ...
     &                         + Atan2(exp1,exp2)
                ...
                depart_phi(i,j,k)=                                      &
     &                       Asin(cos_depart_phi*cos_depart_lambda      &
     &                      * sin_v_latitude(i,j)+                      &
...
! allow for spherical geometry near the poles.
     ...
     &                            *r_theta_levels(i,j,k)) )
                depart_phi(i,j,k) = phi_a(i,j) -                        &
!The following statement cause non reproducibel results on the T3E
!                depart_phi(i,j,k)= (gj-1)*delta_phi + Base_phi -
     &                         (timestep*interpw_wind2(i,j,k))          &
     &                         /(2.0*r_theta_levels(i,j,k))             &
     &                        + tan_theta_latitude(i,j) * weight2 *     &
     &                           interpw_wind1(i,j,k) *                 &
...
! convert back to the old co-ordinate system
     ...
     &                          + Atan2(exp1,exp2)
                ...
                depart_phi(i,j,k)= Asin(cos_depart_phi*cos_depart_lambda&
     &                       * sin_theta_latitude(i,j)+                 &
     &                       sin_depart_phi*cos_theta_latitude(i,j))
              End Do
            End Do
