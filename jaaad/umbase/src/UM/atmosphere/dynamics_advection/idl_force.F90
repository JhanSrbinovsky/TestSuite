...
!
      Subroutine IDL_Force(                                             &
     &                     R, g, kappa, pi, p_zero, row_length          &
...
! -------  ----     -------
! 5.5      17/12/02 This routine introduced                  A.Malcolm
!  6.0   18/08/03  correction to argument list to IDL_FORCE   A.Malcolm
!  6.1   01/08/04  Include different forcing options. R.M.Forbes
!
! Code Description:
...
! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('Force_monsoon',3)
...
! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('Force_monsoon',4)

      Else If (problem_number  ==  dynamical_core) Then

! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('Force_Suarez_Held',3)
...
! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('Force_Suarez_Held',4)

      Else If (problem_number  ==  idealised_problem) Then

! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('IDL_Force_TQUV',3)
...
! DEPENDS ON: timer
        IF (Ltimer) CALL TIMER('IDL_Force_TQUV',4)
