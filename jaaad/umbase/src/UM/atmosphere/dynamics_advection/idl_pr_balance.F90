...
! Haloed arrays are needed to set external haloes for lbcs
        If (L_regular) Then
          do j = 1-halo_j, rows+halo_j
            gj = l_datastart(2) + j - 1
            coslat(j) = cos(Base_phi + (gj-1) * delta_phi)
            y(j) = (gj-1) * delta_y
