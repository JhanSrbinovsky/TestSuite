...
!L Convert ILABEL to IEEE  Integer
      ...
      IEEE_ADDR = 1
#if defined(CRAY)
      IER=CRI2IEG(2,LEN_ILABEL,IEEE_LABEL(IEEE_ADDR),BIT_OFF,ILABEL,    &
     &            1,64,32)
#else
...
#if defined(CRAY)
      IER=CRI2IEG(3,LEN_RLABEL,IEEE_LABEL(IEEE_ADDR),BIT_OFF,RLABEL,    &
     &            1,64,32)
#else
...
#if defined(CRAY)
          IER = CRI2IEG(5,NUM_VALUES-IEXTRAW,IEEE_FIELD,BIT_OFF,FIELD,  &
     &            1,64,32)
#else
...
#if defined(CRAY)
          IER=CRI2IEG(3,NUM_VALUES-IEXTRAW,IEEE_FIELD,BIT_OFF,FIELD,    &
     &            1,64,32)
#else
...
#if defined(CRAY)
          IER=CRI2IEG(2,NUM_VALUES-IEXTRAW,IEEE_FIELD,BIT_OFF,FIELD,    &
     &            1,64,32)
#else
...
! DEPENDS ON: check_extra
          ...
          ENDIF
#if defined(CRAY)
          IER=CRI2IEG(2,1,IEEE_FIELD(IEEE_ADDR),BIT_OFF,FIELD(ADDR),    &
     &            1,64,32)
#else
...
#if defined(CRAY)
          IER=CRI2IEG(3,DATA_VALUES,IEEE_FIELD(IEEE_ADDR),              &
...
!L first check that addr is ok
 ...
 109      FORMAT('CRAY_IEEE: ADDR',I5,1X,'<> NUM_VALUES+1',I5)
          ICODE=1
          RETURN
        ENDIF
!L and so is IEEE_addr
        ...
        IF (IEEE_ADDR /= (NUM_VALUES+1)/2) THEN
          WRITE(CMESSAGE,110)IEEE_ADDR,(NUM_VALUES+1)/2
 110      FORMAT('CRAY_IEEE: IEEE_ADDR ',I5,1X,                         &
     &           ' <> (NUM_VALUES+1)/2',I5)
          ICODE=1
          RETURN
        ENDIF
...
! to act as a delimeter for the M08 software. This extra fields is
! a duplicate,but with a PP field code of -99 .  If MASS is true then last
! record (-99) is omitted.
