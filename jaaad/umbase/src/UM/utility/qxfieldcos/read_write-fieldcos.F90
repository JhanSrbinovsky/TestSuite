...
!LL
!
      SUBROUTINE READ_WRITE(PP_LEN2_LOOKUP,LEN1_LOOKUP,DATA_ADD,        &
     &                      PP_FIXHD,                                   &
...
!     Arguments
      CHARACTER                                                         &
     &     FORMAT_OUT*6                                                 &
                                ! IN    IBM/IEEE/VAX format for output
     &    ,CMESSAGE*80                                                  &
                                ! OUT   error messages
     &    ,OUTFILE*80           ! OUT   pathname of output file
      LOGICAL                                                           &
     &     UNPACK                                                       &
...
!*---------------------------------------------------------------------
!     Called routines
      EXTERNAL READFF,SETPOS,CRAY_IBM,CRAY_VAX,CRAY_IEEE,IOERROR
!*---------------------------------------------------------------------
...
!L -----Having read the LOOKUP table Open the output COS File--------
      CALL GET_FILE(COS_PPUNIT,OUTFILE,80,ICODE)
...
! DEPENDS ON: cray_ibm
     ...
     &                  ICODE,CMESSAGE,LCAL360)
        ELSEIF(FORMAT_OUT == 'VAX') THEN
! DEPENDS ON: cray_vax
          CALL CRAY_VAX(IDIM,NUM_VALUES,PPUNIT,                         &
     &                  LEN1_LOOKUP,PP_LEN2_LOOKUP,PP_FIXHD,LOOKUP,     &
     &                  LOOKUP,ENTRY_NO,DATA_ADD,MODEL_FLAG,            &
     &                  COS_PPUNIT,IEXTRA,IEXTRAW,LAST,OPER,            &
     &                  ICODE,CMESSAGE,LCAL360)
...
! DEPENDS ON: cray_ieee
     ...
     &                   ICODE,CMESSAGE,LCAL360)
#if defined(CRAY)
        ELSEIF(FORMAT_OUT == 'GRIB'.OR.FORMAT_OUT == 'GRIB1'.OR.        &
     &             FORMAT_OUT == 'GRIB2') THEN
          TOTAL_WORDS=0
! DEPENDS ON: cray_grib
          CALL CRAY_GRIB(IDIM,PPUNIT,TOTAL_WORDS,FORMAT_OUT,            &
     &                   LEN1_LOOKUP,PP_LEN2_LOOKUP,PP_FIXHD,LOOKUP,    &
     &                   LOOKUP,ENTRY_NO,DATA_ADD,MODEL_FLAG,           &
