...
!LL
!
      SUBROUTINE CRAY_VAX(IDIM,NUM_VALUES,PPUNIT,                       &
...
!*---------------------------------------------------------------------
!     Called routines
      EXTERNAL READFF,INT_FROM_REAL,CRAY2VAX,TIME2SEC,SEC2TIME
      INTEGER INT_FROM_REAL,CRAY2VAX
!*---------------------------------------------------------------------
!     arguments for called routines
     ...
     &    ,VAX_ADDR                                                     &
                           ! address in VAX fld where extra data going.
     &    ,BIT_OFF                                                      &
...
!                            (32 for odd, 0 for even values of addr)
     ...
     &    ,VAX_LABEL((LEN1_LOOKUP+1)/2) ! holds VAX conversion of LABEL

      REAL                                                              &
     &     FIELD(IDIM)                                                  &
                                  ! array holding data
     &    ,VAX_FIELD(IDIM/2)                                            &
                                  ! array holding VAX data
...
! VAX only has a 32 bit word length compared to the CRAY's 64
! bit word length. In VAX ILABEL(LBLREC) will be no of 32 bit
...
! On output the data will either have been converted to VAX
! numbers and stored in VAX_FIELD or left packed in FIELD,but with
! 32 bit VAX numbers substituted into minimum values.
! If packed then LBLREC/2  words of FIELD are written as LBLREC is
! now the no of 32 bit words. If un-packed VAX_FIELD which has size
...
!     decode LBPACK
      ...
      IF(PACKED) THEN              ! Data left in packed form. Number of
        ILABEL(LBLREC)=ILABEL(LBLREC)*2  ! VAX words needed is 2*CRAY
      ENDIF
...
!L Convert ILABEL to VAX integers.
...
!     data now in vax format
      ILABEL(LBPACK) = ILABEL(LBPACK) - NUMBER_FORMAT*1000 + 5000

!L Convert ILABEL to VAX Integer
      BIT_OFF=0
      VAX_ADDR = 1
! DEPENDS ON: cray2vax
      IER=CRAY2VAX(1,LEN_ILABEL,VAX_LABEL(VAX_ADDR),BIT_OFF,ILABEL)
        IF(IER /= 0) THEN
          ICODE=1
          CMESSAGE=' FUNCTION CRAY2VAX not supported on T3E'
          RETURN
        ENDIF
!L Convert RLABEL to VAX   Real.
      VAX_ADDR=LEN_ILABEL/2
      IF(VAX_ADDR*2 /= LEN_ILABEL) BIT_OFF=32
      VAX_ADDR=VAX_ADDR+1
! DEPENDS ON: cray2vax
      IER=CRAY2VAX(2,LEN_RLABEL,VAX_LABEL(VAX_ADDR),BIT_OFF,RLABEL)
        IF(IER /= 0) THEN
          ICODE=1
          CMESSAGE=' FUNCTION CRAY2VAX not supported on T3E'
          RETURN
      ...
      BIT_OFF=0
      IF(.NOT.PACKED) THEN
!L Convert Real DATA to VAX Real if not packed.
        IF(ILABEL(DATA_TYPE) == 1) THEN         ! Data Type Real
        if(ilabel(32) == 74) then
          WRITE(6,*)'convert type 74 as logical and reset datatype'
! DEPENDS ON: cray2vax
          IER = CRAY2VAX(5,NUM_VALUES-IEXTRAW,VAX_FIELD,BIT_OFF,FIELD)
          ILABEL(DATA_TYPE) = 3
        else
! DEPENDS ON: cray2vax
          IER=CRAY2VAX(2,NUM_VALUES-IEXTRAW,VAX_FIELD,BIT_OFF,FIELD)
          IF(IER /= 0) THEN
            ICODE=1
            CMESSAGE='CRAY_VAX error converting REAL for VAX_FIELD'
            RETURN
          ENDIF
        endif
!L Convert Integer data to VAX Integer.
        ELSEIF(ILABEL(DATA_TYPE) == 2) THEN     ! Data Type Integer
! DEPENDS ON: cray2vax
          IER=CRAY2VAX(1,NUM_VALUES-IEXTRAW,VAX_FIELD,BIT_OFF,FIELD)
          IF(IER /= 0) THEN
            ICODE=1
...
! DEPENDS ON: cray2vax
          ...
          IF(IER /= 0) THEN
            ICODE=1
            CMESSAGE='CRAY_VAX error converting logical for VAX_FIELD'
            RETURN
          ENDIF
        ENDIF
      ELSE
...
!L main while loop that works out code and then checks that code is
!L ok.
!L if code is ok then data_values weill contain the number of REAL entri
!L the vector.
...
! DEPENDS ON: check_extra
          ...
          ENDIF
! DEPENDS ON: cray2vax
          IER=CRAY2VAX(1,1,VAX_FIELD(VAX_ADDR),BIT_OFF,FIELD(ADDR))
!         convert the integer from cray format to VAX format
          IF (IER /= 0) THEN
            ICODE=1
            CMESSAGE='CRAY_VAX: FAILED IN INTEGER CONV OF EXTRA DATA'
            RETURN
          ENDIF

!L         update bit_off, addr and VAX_addr
          IF (BIT_OFF == 0) THEN
            BIT_OFF=32
          ELSE
            BIT_OFF=0
            VAX_ADDR=VAX_ADDR+1 ! gone on another word..
          ENDIF
          ADDR=ADDR+1             ! increment address
!L now to convert REAL vector to VAX format.
! DEPENDS ON: cray2vax
          IER=CRAY2VAX(2,DATA_VALUES,VAX_FIELD(VAX_ADDR),               &
     &      BIT_OFF,FIELD(ADDR))
!         convert the real data values
          IF (IER /= 0) THEN
            ICODE=1
            CMESSAGE='CRAY_VAX: FAILED IN REAL CONV OF EXTRA DATA'
            RETURN
          ENDIF
!L update loop variables.
          ADDR=ADDR+DATA_VALUES
          VAX_ADDR=VAX_ADDR+DATA_VALUES/2
...
!L Verify addr and VAX_addr have correct values at end of whileloop
!L first check that addr is ok
        IF (ADDR /= NUM_VALUES+1) THEN
          WRITE(CMESSAGE,109)ADDR,NUM_VALUES+1
 109      FORMAT('CRAY_VAX: ADDR',I5,1X,'<> NUM_VALUES+1',I5)
          ICODE=1
          RETURN
        ENDIF
!L and so is VAX_addr
        IF (BIT_OFF == 0) VAX_ADDR=VAX_ADDR-1
        IF (VAX_ADDR /= (NUM_VALUES+1)/2) THEN
          WRITE(CMESSAGE,110)VAX_ADDR,(NUM_VALUES+1)/2
 110      FORMAT('CRAY_VAX: VAX_ADDR ',I5,1X,' <> (NUM_VALUES+1)/2',I5)
          ICODE=1
          RETURN
        ENDIF
      ...
      IF(PACKED) THEN
        WRITE(COS_PPUNIT) VAX_LABEL
        WRITE(COS_PPUNIT) (FIELD(I),I=1,ILABEL(LBLREC)/2)
!       WRITE(6,111) VAX_LABEL
!       WRITE(6,111) (FIELD(I),I=1,50)
      ELSE
        WRITE(COS_PPUNIT) VAX_LABEL
        WRITE(COS_PPUNIT) VAX_FIELD
!       WRITE(6,111) VAX_LABEL
!       WRITE(6,111) (VAX_FIELD(I),I=1,50)
      ENDIF
  111 FORMAT(1X,5Z16)
!
