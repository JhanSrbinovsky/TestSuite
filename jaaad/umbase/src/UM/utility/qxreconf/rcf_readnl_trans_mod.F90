...
!   6.0   07/01/04   Remove #if def protection from rewind command as
!                    it's also required on NEC. R.Sharp
!
! Code Description:
!   Language: FORTRAN 90
...
!-----------------------------------------------------------------
! The T3E's and the SX6 require this rewind, the HP's don't
!-----------------------------------------------------------------
Rewind( Unit=nft )
...
Do While ( iostatus == 0 )
  ...
  If (iostatus == 0 ) Then
    ...
    If ( num_trans > max_trans ) Then
      Cmessage = 'Maximum number of transplanted fields exceeded - &
                 &please use a mod to increase'
      Write (6,*) Cmessage
