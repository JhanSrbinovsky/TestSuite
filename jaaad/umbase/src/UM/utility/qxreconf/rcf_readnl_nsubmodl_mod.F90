...
!
! Method:
!   Data read into the Rcf_Submodel_Mod module.
!
...
Use Rcf_Submodel_Mod   ! Almost all of it...
...
If ( N_Internal_Model > N_Internal_Model_Max ) Then
   Write (Cmessage,*) 'UM_Submodel_In: FATAL ERROR. Too many internal',&
...
Do i = 1, N_Internal_Model ! loop over internal models
  ...
  If (im <= 0 .OR. im >= Internal_ID_Max ) Then
   Write(Cmessage,*) 'Illegal internal ', &
        'model identIfier=',im, &
  ...
  If (sm <= 0 .OR. sm >= Submodel_ID_Max ) Then
   Write(Cmessage,*) 'UM_Submodel_In: FATAL ERROR. Illegal submodel ', &
        'dump identIfier=',sm, &
...
If (N_Submodel_Partition > N_Submodel_Partition_Max ) Then
   Write(Cmessage,*) 'UM_Submodel_In: FATAL ERROR.Too many submodels ',&
