...
!   5.3   29/01/02   Using function get_umversion to get um version
!                    number from environment variable $VN. D.M. Goddard
!
! Code Description:
!   Language: FORTRAN 90
...
Subroutine HdAncilM ( AncMsrNam , EnvDir )
...
! DEPENDS ON: get_um_version
      ...
      If (char1 .eq. 'H') Then
        ...
        If (char1 == '1') Then   !  H1 found in header
          ...
          if (.not. found_H1) Then  ! Invalid H1 header found ?
            Write (cmessage,*) 'Invalid Sub-Model No ',c_model_no, &
  ...
  If (.not. found_H1 ) Then
    Write (Cmessage,*) 'H1 header not found in AncilMaster file.'
    ErrorStatus = 70
    Call Ereport ( RoutineName, ErrorStatus, cmessage )
  End If
  If (.not. found_H4 ) Then
    Write (Cmessage,*) 'H4 header not found in AncilMaster file.'
    ErrorStatus = 80
    Call Ereport ( RoutineName, ErrorStatus, cmessage )
  End If
