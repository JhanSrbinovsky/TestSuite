...
!   addressing.
!
!   Derived from UM 4.5 code with model specifics stripped out.
!
...
!-------------------------------------------------------------------
! This routine determines the Adressing of the output dump for
! reconfiguration - includes calls to primary etc to determine
! which fields are included.
!--------------------------------------------------------------------

SUBROUTINE Rcf_Address( )
      ...
      If(Im_ident == SUBMODEL_IDENT) Then
        ...
        Do ISEC = 0,34
          If (ISEC == 0  .OR.    & ! Prognostic variables
              ISEC == 3  .OR.    & ! Boundary Layer variables - kdcorbin, 05/10
              ISEC == 33 .OR.    & ! Free tracers
              ISEC == 34 .OR.    & ! UKCA tracers
              Var_Recon) Then      ! VAR reconfiguration
...
! If level type is 0, is LBC so multiply size by levels
    ...
    If (ISPACE /= 4 .AND. ISPACE /= 9) Then
      If (ISEC == 0 .OR. &
          ISEC == 3 .OR. &  !kdcorbin, 05/10
          ISEC == 33 .OR. ISEC == 34 .OR. VAR_RECON) Then
