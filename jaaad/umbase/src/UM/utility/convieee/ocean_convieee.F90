...
!----------------------------------------------------------------------
      ...
      SUBROUTINE OCEAN_CONVIEEE(NFTIN,NFTOUT,IEEE_TYPE,MAX_FIELD_SIZE   &
     &,                         LEN_FIXHD,LEN_INTHD                     &
...
#include "argppx.h"
     &                          WGDOS_EXPAND)
      ...
      INTEGER LOOKUP_LBLREC_NEW(LEN2_LOOKUP)
      INTEGER LOOKUP_LBEGIN_NEW(LEN2_LOOKUP)
      INTEGER LOOKUP_LBNREC_NEW(LEN2_LOOKUP)
      INTEGER LOOKUP(LEN1_LOOKUP,LEN2_LOOKUP)
! Local arrays:--------------------------------------------------------
      INTEGER D1(MAX_FIELD_SIZE) !Data array used to read in each fieldl
      REAL   IEEE_64(MAX_FIELD_SIZE) !Array containing 64 bit IEEE data
      REAL   C1(JOC_NO_SEAPTS)       !Array holding compressed field
      REAL   E1(JOC_NO_SEAPTS)       !Array holding compressed field
      REAL   U1(LEN_OCFLD)           !Array holding uncompressed field
! External subroutines called:-----------------------------------------
      EXTERNAL ABORT,READFLDS,ABORT_IO,BUFFO32,PR_LOOK,UNPACK
...
! Local variables:-----------------------------------------------------
      ...
      INTEGER      NCOMP    ! Number of compressed fields
      ...
      INTEGER      RECNUM   ! Record number of field in lookup table
      INTEGER      POSIN    ! Start position of field within C1
      INTEGER      IEEE2IEG
      INTEGER      INT_FROM_REAL ! Function to convert real to int
      REAL         A        ! Return code from BUFFIN; -1.0 = O.K.
      LOGICAL      LL_CYCLIC! T => cyclic ; f=> not cyclic
...
! 1: Process ocean compressed fields first

      NCOLS  = INTHD(6)
      NROWS  = INTHD(7)
      NLEVS  = INTHD(8)
      IF(FIXHD(12) >= 304)THEN
        NT     = INTHD(14)
      ELSE
        NT     = INTHD(14)+2
      END IF
...
! Decide whether there are any compressed fields and how many of them.
! Use LBPACK to determine whether the first field contains sea points
! only
      IF( MOD(LOOKUP(LBPACK, 1)/10,10)  ==  0) THEN
        NCOMP = 0
      ELSE
        NCOMP = NT + 2
!Can only convert compressed fields from cray pvp format to
!                                     64-bit ieee format
          WRITE(6,*) 'CONVIEEE: Conversion not supported'
          WRITE(6,*) 'Conversion of compressed ocean fields to '        &
     &,              '32-bit IEEE format not supported'
! DEPENDS ON: ereport
          CALL EREPORT('OCEAN_CONVIEEE', 1000,                          &
...
! Loop over levels storing all levels in one 1-D array and convert
        POSIN = 1
        DO J = 1,NLEVS

! Reset the headers in case WDGOS packing has altered them
        IF(WGDOS_EXPAND == 1) THEN
          LOOKUP(LBLREC,J+(L-1)*NLEVS)=LOOKUP_LBLREC(J+(L-1)*NLEVS)
          LOOKUP(LBEGIN,J+(L-1)*NLEVS)=LOOKUP_LBEGIN(J+(L-1)*NLEVS)
          LOOKUP(LBNREC,J+(L-1)*NLEVS)=LOOKUP_LBNREC(J+(L-1)*NLEVS)
          FIXHD(160)=OLD_FIXHD_160
        ENDIF

! DEPENDS ON: readflds
          CALL READFLDS(NFTIN,1,RECNUM,LOOKUP,LEN1_LOOKUP,              &
     &                  E1(POSIN),MAX_FIELD_SIZE,FIXHD,                 &
#include "argppx.h"
     &                  WGDOS_EXPAND,ICODE,CMESSAGE)
! DEPENDS ON: abort_io
          IF(ICODE /= 0)CALL ABORT_IO('CONVIEEE',CMESSAGE,              &
     &                                ICODE,NFTIN)
          POSIN=POSIN+LOOKUP(LBLREC, J+(L-1)*NLEVS)
          RECNUM=RECNUM+1
        END DO

! Convert to IEEE format.
        ITYPE = 2          !Assume compressed data are type 'real'
! Uncompress 3-D field
! DEPENDS ON: unpack
        CALL UNPACK(1,NROWS,1,NLEVS,NROWS,NLEVS,NCOLS,NROWS,NLEVS,      &
     &              CFI1,CFI2,LEN_CFI1,CFI3,JOC_NO_SEAPTS,              &
     &              C1,U1,RMDI,LL_CYCLIC)

! Write uncompressed IEEE data to disk a level at a time
        DO J = 1,NLEVS
          DO I=1,LOOKUP_LBLREC_NEW(J+(L-1)*NLEVS)
            IEEE_64(I)=U1(I+(J-1)*NROWS*NCOLS)
          END DO     !I
! DEPENDS ON: setpos
          CALL SETPOS(NFTOUT,LOOKUP_LBEGIN_NEW(J+(L-1)*NLEVS),K)
! DEPENDS ON: buffout
          CALL BUFFOUT(NFTOUT,IEEE_64,                                  &
     &                 LOOKUP_LBNREC_NEW(J+(L-1)*NLEVS),                &
     &                 LEN_IO,A)

! Check for I/O errors
          IF(A /= -1.0.OR.                                              &
     &       LEN_IO /= LOOKUP_LBNREC_NEW(J+(L-1)*NLEVS))THEN
! DEPENDS ON: ioerror
            CALL IOERROR('buffer out of data field'                     &
     &,                  A,LEN_IO                                       &
     &,                  LOOKUP_LBLREC_NEW(J+(L-1)*NLEVS))
! DEPENDS ON: ereport
            CALL EREPORT('OCEAN_CONVIEEE', 1002,                        &
     &       'Buffer of out data field wrong size')

          END IF

          WRITE(6,'(''Field '',i5,'' : Stash Code '',i5,                &
     &              '' : has been converted'')')                        &
     &       J+(L-1)*NLEVS,LOOKUP(ITEM_CODE,J+(L-1)*NLEVS)
...
! 2. Process uncompressed ocean fields

! Loop over all fields
      DO I=NCOMP*NLEVS+1,LEN2_LOOKUP
...
! DEPENDS ON: ereport
            CALL EREPORT('OCEAN_CONVIEEE', 1005,                        &
