...
! Method:
!
! Current Code Owner: D Robinson / I Edmond
!
! Code Description:
...
!   Scalar arguments with intent(in):
      ...
      integer field_types      ! number of field types in I/O files
...
! Local parameters:
      ...
      parameter (max_len1_rowdepc=800)
      parameter (max_len2_rowdepc=5)
      parameter (max_len1_coldepc=1082)
...
! Local Scalars
     ...
     &,number_of_data_words_in_memory  ! Number of Data Words in memory
      ...
      character*80 cmessage    ! error message from WRITHEAD
      character*80 ancfile, levels
...
!L
      if (wave .and. compress) then
        ...
        call get_file(ftin2,LEVELS,80,icode)
        levels=trim(levels)
...
!L 1.0 Open the UM/ancillary file
      ...
      call get_file(ftout,ANCFILE,80,icode)
! DEPENDS ON: file_open
      call file_open (ftout,ANCFILE,80,1,1,icode)
      if (icode >  0) then
...
!L    file headers.  Loop over n_times then field_types.
...
! DEPENDS ON: readdata
   ...
   10 continue    ! end of loop over levels

   15 continue    ! end of loop over field_types

   20 continue    ! end of loop over times
...
!    & len1_lookup_all,len2_lookup,len_data,start_block,icode,
!    & cmessage)
