...
! Local Scalars
      ...
      integer ierr                ! error status from ibm2cri
...
! DEPENDS ON: to_land_points
            CALL to_land_points(data_field,field_to_write,lsmask,       &
...
! DEPENDS ON: decompose_smexe
      CALL DECOMPOSE_SMEXE(columns, rows,0,0,1)
