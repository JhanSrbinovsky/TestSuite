...
!     FieldEQ: Set MDI where two fields are not equal.
...
!     Method:
!     -------  loop over all elements, testing unless either is
...
! ------------------------------------------------------------------
      ...
      icode =0
      cmessage="FieldNE: test successful."
     ...
     &          .AND. in_field1(ix,iy) == in_field2(ix,iy) )            &

     &      THEN

              out_field(ix,iy) = in_field1(ix,iy)
