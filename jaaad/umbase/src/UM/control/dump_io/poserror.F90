 ...
 || defined(FLDMOD) || defined(VAROPSVER)
! *****************************COPYRIGHT*******************************
...
!
      write(0,'(//)')
      WRITE(0,'('' ******FATAL ERROR WHEN READING MODEL DUMP******'')')
      WRITE(0,'('' Conflict between start position of '',A)')STRING
      WRITE(0,'('' Block and Pointer in Fixed Length Header: FIXHD('',  &
     &I3,'') ='',I9)')HEAD_POS,HEAD_ADDRESS
      WRITE(0,'('' Current position in file ='',I9,'' words in'')')     &
     &START_BLOCK
      WRITE(0,'('' ***********************************************'')')
      call abort('I/O Position Error')
#endif
