...
!*L Arguments:-------------------------------------------------
     ...
     &                      ERROR)
      INTEGER                                                           &
     &   GRIB_BLOCK1_OCTET4                                             &
                              ! OCTET 4 FROM GRIB PDB        INPUT
     &  ,GRIB_BLOCK1_OCTET9                                             &
                              ! OCTET 9 FROM GRIB PDB        INPUT
     &  ,STASH_SECTION_NUMBER                                           &
                              ! STASH SECTION NUMBER         OUTPUT
     &  ,STASH_ITEM_NUMBER                                              &
                              ! STASH PARAMETER VALUE        OUTPUT
     &  ,ERROR                ! ERROR OUTPUT CODE            OUTPUT
!     LOCAL VARIABLES
      INTEGER                                                           &
     &   CARRY   ! CARRY VALUE FROM ODD VALUES OF GRIB_BLOCK1_OCTET4
!****
      IF(GRIB_BLOCK1_OCTET4 <  128.OR.GRIB_BLOCK1_OCTET4 >  253) THEN
        ERROR = 99
        RETURN
      ENDIF
