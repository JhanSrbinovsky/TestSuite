...
!LL  Routine: STGLOM ---------------------------------------------------
!LL
!LL  Purpose: Calculate weighted global mean within a region specified
!LL           by a lower left hand and upper right hand corner.
!LL           Multiple level fields.
...
!LL                  R Rawlins.
!LL   5.1  21/12/99  Swap north/south arguments into STASH_GATHER_FIELD.
!LL                  If there is no mass weighting, ensure only first
!LL                  level of pstar_weight is accessed (set to 1.0).
!LL                  R Rawlins
!    5.3  07/03/02 add code to initialise k       Andy Malcolm
...
!LL
!LL  Logical components covered: D716
!LL
!LL  Project task: D7
!LL
...
!
        INTEGER i,ii,j,k,kk                                             &
                            ! ARRAY INDICES FOR VARIABLE
     &,kkk    ! vertical index for pstar_weight
...
#include "parvars.h"
      ...
      INTEGER                                                           &
! Co-ords to PE for first point of subarea (ie bottom left = SW corner)
     &  proc_start_x , proc_start_y                                     &

! unused return values from GLOBAL_TO_LOCAL_RC
     &, dummy1 , dummy2                                                 &

! PE number of PE at origin of subarea
     &, sum_pe                                                          &
...
! sum(1) is equivalenced to SUMGBOT
! sum(2) is equivalenced to SUMGTOP
      REAL sum(2)

      EQUIVALENCE                                                       &
     &  (sum(1) , SUMGBOT ) , (sum(2) , SUMGTOP)
...
! miss halos on wrap around
     ...
     &              fieldin(ii,j,k)*pstar_weight(ii,j,kkk)*             &
     &              area_weight(ii,j)
              ENDIF
            ENDDO
          ENDDO
...
! (Only the PE at the subdomain origin [=SW corner] gets the field mean)
...
! DEPENDS ON: global_to_local_rc
     ...
     &  dummy1,dummy2)
      ...
      sum_pe=proc_start_x + nproc_x*proc_start_y
...
! miss halos on wrap around
     ...
     &          fieldin(ii,j,k)*pstar_weight(ii,j,kkk)*area_weight(ii,j)
        ...
        ENDDO ! kk : loop over levels
      ENDIF ! if subdomain covers this processor

! 3.0  add all the partial sums together, and store

! sum(1) is equivalenced to SUMGTOP
! sum(2) is equivalenced to SUMGBOT
