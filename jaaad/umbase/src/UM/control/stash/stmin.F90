...
!LL  Routine: STMIN ----------------------------------------------------
!LL
!LL  Purpose: Computes the point-by-point minimum in time of a field
!LL           by comparing the field at the current time with the
!LL           minimum so far (STASH TEMPORAL service routine)
!LL
...
!L     result to amdi, else set result to minimum of fieldin and result
!L
      IF (masking) THEN
        DO i=1,size
      ...
      ELSE
!L
!L 1.2 loop over array size, set result to minimum of fieldin and result
