...
! GCOM version of swap_bounds to deal with multiple variables
! at once
...
!  better. This is the GCOM version.
!
...
!         processor.
  ...
  ELSE ! more than 2 processors East-West
    ...
    IF (neighbour(PEast) /= NoDomain) THEN
      ...
      ! Send East
       CALL GC_RSEND(3, length, neighbour(PEast), info,                   &
    ...
    IF (neighbour(PWest) /= NoDomain) THEN
      ...
      ! Send West
      CALL GC_RSEND(2, length, neighbour(PWest), info,                    &
                    receive_buffer, send_buffer)
    ...
    IF (neighbour(PEast) /= NoDomain) THEN
      ...
      ! Receive from East
      CALL GC_RRECV(2, length, neighbour(PEast), info,                    &
                    receive_buffer, send_buffer)
    ...
    IF (neighbour(PWest) /= NoDomain) THEN
      ...
      ! Receive from West
      CALL GC_RRECV(3, length, neighbour(PWest), info,                    &
                    receive_buffer(index_2_start),                        &
                    send_buffer(index_2_start))
...
ELSE                 !!! bc_cyclic in NS
  ...
  IF (at_extremity(PSouth)) THEN
    ...
    IF ((neighbour(PSouth) /= NoDomain) .AND.                             &
       ...
       CALL GC_RSEND(11, length, neighbour(PSouth), info,                 &
                     receive_buffer(index_2_start),                       &
                     send_buffer )
  ...
  ELSE ! not at the South
     ...
     CALL GC_RSEND(12, length, neighbour(PSouth), info,                   &
                   receive_buffer, send_buffer)
  ...
  IF (at_extremity(PNorth)) THEN
    ...
    IF ((neighbour(PNorth) /= NoDomain) .AND.                             &
       ...
       CALL GC_RSEND(13, length, neighbour(PNorth), info,                 &
                     receive_buffer,                                      &
                     send_buffer(index_2_start) )
  ...
  ELSE
       ...
       CALL GC_RSEND(14, length, neighbour(PNorth), info,                 &
                     receive_buffer(index_2_start),                       &
                     send_buffer(index_2_start) )
  ...
  IF (at_extremity(PSouth)) THEN
    ...
    IF ((neighbour(PSouth) /= NoDomain) .AND.                             &
       ...
       CALL GC_RRECV(11, length, neighbour(PSouth), info,                 &
                     receive_buffer(index_2_start),                       &
                     send_buffer)
  ...
  ELSE ! not at the South
       ...
       CALL GC_RRECV(14, length, neighbour(PSouth), info,                 &
                     receive_buffer(index_2_start),                       &
                     send_buffer(index_2_start))
  ...
  IF (at_extremity(PNorth)) THEN
    ...
    IF ((neighbour(PNorth) /= NoDomain) .AND.                             &
     ...
     CALL GC_RRECV(13, length, neighbour(PNorth), info,                   &
                   receive_buffer,                                        &
                   send_buffer(index_2_start))
  ...
  ELSE ! not at the North
     ...
     CALL GC_RRECV(12, length, neighbour(PNorth), info,                   &
                   receive_buffer, send_buffer)
