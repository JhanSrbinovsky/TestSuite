...
TYPE (UM_header_type), INTENT (OUT) ::  UMhdr ! UM header from dump
...
EXTERNAL SETPOS, BUFFIN       ! Used to move position within Model dump
...
! DEPENDS ON: setpos
    ...
    DO
! DEPENDS ON: buffin
      CALL BUFFIN( UMhdr % UnitNum, Lookup(1), UMhdr % FixHd( 151 ), &
              Len_IO, a_IO )
      ...
      IF ( FFLookup > 4096 ) THEN
        Cmessage = 'FieldsFile has > 4096 Lookups'
        ReturnCode = 1
        CALL Ereport( RoutineName, ReturnCode, Cmessage )
      END IF
    END DO
