...
! ----     -------     -------
! 09/06/00 5.1     Filled external halos of "p" with sensible numbers
!                                                            P.Burton
! 22/03/00 5.1        Changed j_start/stop for cyclic LAM  Andy Malcolm
!LL   5.1   11/02/00  Use DOMTYP parameters                    P.Burton
!LL   5.2   10/10/00  fix problem in GCR_k if RHS = 0     A.Malcolm
!   5.2   23/10/01    Change from magic numbers         Andy Malcolm
...
!  6.1   02/08/04  Remove L_vector                        Terry Davies
!  6.2   11/01/06  Added Iteration count diagnostic routines
!                                                         T.Edwards
!  6.2   21/10/05  Remove unused external                 Paul Selwood
!  6.2  03/11/04  change dimensions to allow loop collapsing in
!                 Mpp_tri_solve_exec.   Klaus Ketelsen/R Barnes
!  6.2   04/10/05  Modify iteration printing       Terry Davies
!
! Code Description:
...
!
      ...
      Integer, Save :: GCR_iterations(max_numcycles) = 1  
      ...
      Integer, Save :: GCR_iterations_min(max_numcycles) = 1000  
                                ! Previous min
...
!     External Routines:
      ...
      External                                                          &
     &  GCR_Elliptic_Operator                                           &
     &, GCR_Coefficient                                                 &
     &, GCR_Two_Norm                                                    &
     &, GCR_precon_1_setup                                              &
     &, GCR_precon_1_exec                                               &
     &, GCR_precon_ADI_setup                                            &
     &, GCR_precon_ADI_exec                                             &
     &, GCR_precon_ADI_setup_trisolve                                   &
     &, GCR_precon_ADI_exec_trisolve                                    &
     &, GCR_calc_abs_norm                                               &
     &, GCR_error_print                                                 &
     &, GCR_abs_print
...
! DEPENDS ON: gcr_precon_adi_setup_trisolve
          Call GCR_precon_adi_setup_trisolve(j_start_c,j_end_c,         &
     &                  HM_Cxx1, HM_Cxx2, HM_Cyy1, HM_Cyy2,             &
     &                  HM_Czz, HM_Cz, HM_C3, HM_C4,                    &
     &                  eta_theta_levels, eta_rho_levels,               &
     &                  r_theta_levels, r_rho_levels,                   &
...
!!      parallel variables added
     ...
     &                  )
      ...
      Else
        If (L_gcr_fast_x) Then
! DEPENDS ON: gcr_precon_adi_exec_trisolve
        Call GCR_precon_adi_exec_trisolve(j_start_c,j_end_c,            &
...
! DEPENDS ON: fill_external_halos
      CALL FILL_EXTERNAL_HALOS(p(1-offx,1-offy,1,n_restart+1),          &
     &                         row_length,rows,                         &
     &                         model_levels,Offx,Offy)
...
! DEPENDS ON: gcr_abs_print
 ...
 999  FORMAT(' Iterations: Max =',I4,' at timestep ', I6,               &
     &                  '. Min =',I4,' at timestep ', I6)
      ...
      return    !  end of subroutine gcr_k
