...
!   5.2   18/12/00  fix bug in communication on demand       A.Malcolm
!   5.2   18/1/01   fix in extending data, scheme 4          A.Malcolm
...
! Arguments with Intent IN. ie: Input variables.
     ...
     &, n_rows                                                          &
                        ! rows for v-field dynamic arrays
     &, rows_g                                                          &
                        ! rows for input field dynamic arrays
     &, g_row_length                                                    &
...
! subroutines
     ...
     &,        Extend_data_quintic
      ...
      External Mono_Conserv, eta_vert_weights, eta_vert_weights_e
...
!cdir collapse
     ...
     &                              glambda_u(i_out_w(i,j,k)) ) then
              ...
              i_out(i,j,k) = i_out_w(i,j,k)
            endif ! depart point is before u(i)
     ...
     &                      phi_v(i, j_out_w(i,j,k)) )then
                   ...
                   j_out(i,j,k) = j_out_w(i,j,k)
            endif        ! depart point is before v(j)
...
! DEPENDS ON: mono_conserv
     ...
     &                       Data_out3, conserv_fail)
        ...
        ElseIf ( type == 2 ) then  ! for monotone v
