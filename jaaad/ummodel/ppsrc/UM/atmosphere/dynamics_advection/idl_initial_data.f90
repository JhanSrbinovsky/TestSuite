...
! *****************************COPYRIGHT*******************************
!
! Subroutine IDL_Initial_data

      Subroutine IDL_Initial_data(                                      &
     &                      R, g, kappa, epsilon, Cp                    &
     &,                     p_zero, Earth_radius, Pi, two_omega         &
...
! -------  ----     -------
! 5.3     10/10/01  This routine introduced            Terry Davies
! 5.4     29/08/02  Changes to random number generation  Carol Roadnight
! 5.4     29/08/02  Remove STOP and replace with a warning
!                   if qprofile=9 and orography.    Carol Roadnight
!  5.4  20/04/02  Include L_trivial_trigs in initial data to enable
!                 balanced pressure option for Cartesian grid
!                                                        Terry  Davies
! 5.5     03/02/03  Re-arrange u,v wind code (section 5), include
!                   option to read u,v profile from namelist and
!                   include call to deformation front setup routine.
! 5.5     03/02/03  Include qcf2,qrain,qgraup and set to 0. R.M.Forbes
! 6.2    31/01/06  Pass idl bubble variables through to new routine
!                  Idl_initialise_bubble.  Richard Forbes.
! 6.2    31/01/06  Place random number generation code into a routine.
!                                                      R.M.Forbes
! 6.1     09/07/04  Correct pointers for L_rotate_winds=.T.  R.M.Forbes
! 6.1    04/10/04  fix random number problem.
!                  fix bit reproducibility for perturbed IC's
!                                                      A. Malcolm
! 6.2     15/08/05  Free format fixes. P.Selwood.
!  6.2   31/12/05  Variable resolution changes      Terry Davies
! 6.2     16/01/06  Include subroutines for cyclone and
!                   baroclinic wave experiments  Bob Beare
! 6.2    05/01/06  Use u,v height array for namelist profile. R.Forbes
! 6.2    05/01/06  Put initial humidity code into a subroutine and
!                  introduce q reference profile array. R.Forbes
! 6.2    03/01/06  Improve idealised runtime print output. R. Forbes.
! 6.2    31/01/06  Add call to IDL_calc_exner. Yongming Tang
! 6.2    31/01/06  Add option for interpolation method. Yongming Tang
! 6.2    31/01/06  Change qcl to theta_eh, qcf to rho_eh and
!                  w_adv to exner_rho_levels_eh. Richard Forbes
!
! Code Description:
...
!-------------------------------------------------------------------
      ...
      Else If (tprofile_number  ==  tp_dyn_core_lam) Then
        ...
        If (model_domain  /=  mt_lam) Then
          Print*,'ERROR: tProfile allowed for LAM domain ONLY '
          Print*,'ERROR: tProfile_number = ',tprofile_number
          Stop
        Endif
...
! DEPENDS ON: swap_bounds
      CALL SWAP_BOUNDS(theta_eh, row_length, rows, model_levels,        &
     &                 halo_i, halo_j, fld_type_p,.FALSE.)
...
!    Check for silly pressure at top
      ...
      if(p_test  <   0.0) then
        print*,'** Pressure negative at model top   ***'
        print*,'**   Cannot run    ***'
        stop
      endif   !(p_test  <   0.0)
...
! DEPENDS ON: idl_calc_exner
     ...
     &,                     r_theta_levels, r_rho_levels                &
     &,                     eta_theta_levels, eta_rho_levels            &
     &,                     theta_eh, q, exner_rho_levels_eh            &
     &,                     exner_theta_levels                          &
     &,                     height_domain                               &
...
! DEPENDS ON: idl_initialise_bubble
     ...
     &,                     r_theta_levels, Earth_radius                &
     &,                     recip_Kappa, p_zero, me, l_datastart        &
     &,                     global_row_length, global_rows              &
     &,                     idl_max_num_bubbles, idl_bubble_option      &
     &,                     idl_bubble_max, idl_bubble_height           &
     &,                     idl_bubble_xoffset, idl_bubble_yoffset      &
...
! DEPENDS ON: w_lltoeq
     ...
     &         work3, work4, u_field_size, u_field_size)
...
! DEPENDS ON: w_lltoeq
     ...
     &         work7, work8, v_field_size, v_field_size)
...
! DEPENDS ON: idl_pr_balance
          Call IDL_pr_balance(                                          &
     &                      R, g, kappa, epsilon, Cp, two_omega         &
     &,                     p_zero, Earth_radius, Pi                    &
     &,                     model_domain, row_length, rows, n_rows      &
     &,                     model_levels                                &
...
!-------------------------------------------------------------------
     ...
     &   k, temp, temp1/100., '[Nominal rho-height]'
        Do k = model_levels , 1, -1
          ...
          Write(Unit=6,Fmt='(4X,I3,4X,F8.2,3X,F7.2,2X,F6.2,             &
     &                       2X,F6.2,3X,F6.4)')                         &
     ...
     &              'Settings ======================='
      ...
      Return
      END SUBROUTINE IDL_Initial_data
!  End subroutine IDL_Initial_data
