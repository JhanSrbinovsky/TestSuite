...
!
      ...
      SUBROUTINE DEEP_CONV(nbl,nlev,ntra,n_cca_lev,n_dp,trlev,          &
                             ...
                             W_MAX, wstar, zlcl_uv,                     &
                             freeze_lev,recip_pstar,qse, ad_on,mdet_on, &
                             ent_on, sdet_on, new_termc,                &
...
!
      ...
      Use cv_run_mod, Only:                                             &
          l_mom, l4a_kterm, l_eman_dd, cape_opt, cape_ts_w, cape_min,   &
...
!
      ...
      real, intent(out) :: up_flux_half(n_dp,nlev)
                                      ! Updraught mass flux
                                      ! on rho levels (Pa/s)
...
!
      ...
      real :: rbuoy_p_old(n_dp)      !buoyancy excess on previous
                                     !level
...
!      real :: ttkm1(n_dp)             ! Temporary store for T in layer
      ...
      logical :: bwkp1(n_dp)          !mask for liquid condensate on
                                      !k+1
...
! Required by all version of CMT
      ...
      Logical :: l_mom_gk=.false.             ! true if Gregory-Kershaw CMT required

!!!!  added by Hongyan Zhu 
      Logical :: l_mom_hy=.false.
...
! C_MASS end
!  WCAPE_FAC Start 
!   Tuneable factor used in denominator of W_CAPE timescale calcn 
...
! DEPENDS ON: layer_cn
        Call LAYER_CN(k,n_dp,n_dp,nlev,                                 &
...
!and needs to be compressed to be fed into conv24a and hence parcel
...
! limited to timestep (operational and alternative schemes the
! same for deep convection).
          If (nterm >  0) then
            Do j = 1,nterm
              i = index_nterm(j)
              If (dcpbydt(i)  >   0.0) then
...
! variable (cape_out).
        ...
        End if
!end test on setting for CAPE closure choice 0, 1 or 2
...
! DEPENDS ON: dd_all_call
     ...
     &,                      l_tracer, flg_dwn_flx                      &
     &,                      flg_entr_dwn, flg_detr_dwn, bwater(1,2)    &
     &,                      exner_layer_centres,exner_layer_boundaries &
...
!------------------------------ -----------------------------------------
      ...
      If (L_mom) then
!changed by Hongyan Zhu       Select Case (deep_cmt_opt)
       Select Case (2)
        Case (2)       ! Gregory-Kershaw deep CMT

          ! Do nothing here as calculated in above parcel ascent     

        Case Default   ! (0/1) Alan Grants Eddy viscosity CMT
