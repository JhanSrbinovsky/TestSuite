...
! ----------------------------------------------------------------------
     ...
     &      diffusion_order(k)  >   0 )Then
          ...
          Do j = 1-off_y, rows+off_y
            ...
            End Do
          End Do
! calculate D(r)/D(eta) about u levels
          if(k  <   model_levels)then
          Do j = 1-off_y, rows+off_y
            Do i = 1-off_x, row_length+off_x
            drdeta(i,j,n_active_levels) = (r_theta_at_u(i,j,k) -        &
     ...
     &              model_levels + horizontal_level
      ...
      Do ka = 1, level_flat - 1
        ...
        Do j = 1, rows
          Do i = 0, row_length
            if(k == 1)then
              lower_surface=r_theta_at_u(i,j,0)
            else
...
! End of routine H_cdiff_u
      return
      END SUBROUTINE H_cdiff_u
